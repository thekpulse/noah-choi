...
(() => {
  ...
  /* 🔴 v26.1 재타겟(원칙 128) — 넷째 갈래에 **주어를 넣었습니다**
     (「아직 영수증에 없어요」 → 「아직 **인테리어비가** 영수증에 없어요」 · 오너 지적).
     ⏹ 겨눔이 문장을 통째로 잡고 있어서 주어를 붙이는 정당한 개선이 막혔습니다(원칙 48).
     ⚠ 그리고 `{0,400}`이 **주석까지 세고 있었습니다** — 근거 주석을 한 줄 더 달면
       겨눔이 갈래를 다 못 읽고 빈손이 됩니다. 주석을 먼저 걷어내고 셉니다.
     → 잠글 사실만 남깁니다 : **네 갈래가 「영수증에 들어갔는가 / 아닌가」를 다 말하는가.** */
  tt('인테리어가 영수증에 들어갔는지 화면이 말한다', (()=>{
     const i = UI.indexOf("$('itKicker').textContent =");
     if(i < 0) return false;
     const blk = UI.slice(i, UI.indexOf(';', i) + 1).replace(/\/\*[\s\S]*?\*\//g, '');
     return /영수증에 없어요/.test(blk) && /영수증에 더했어요/.test(blk) && /영수증에 더해요/.test(blk);
  })());
  /* 🔴 v26.1 신설 — **없다고 말하는 갈래가 「무엇이」 없는지 말하는가**(오너 지적 ·
     사모님이 결과 화면에서 잡은 것과 같은 종류). 주어가 다시 떨어지면 빨간불입니다. */
  tt('인테리어 킥커가 무엇이 없는지 말한다', (()=>{
     const i = UI.indexOf("$('itKicker').textContent =");
     if(i < 0) return false;
     const blk = UI.slice(i, UI.indexOf(';', i) + 1).replace(/\/\*[\s\S]*?\*\//g, '');
     return /인테리어비가 영수증에 없어요/.test(blk);
  })());
  ...
  /* 🔴 v25.41 — **대상을 옮겼습니다**(원칙 128). 전에는 「`<b>주택담보대출</b>` … `<b>주담대</b>」
     라는 **문장 꼴**을 잠갔습니다. 그 문장(「을 줄여 … 라고 적어요」)이 실기에서 **네 줄로 토막나
  ...
  /* 🔴 v26.0 재타겟(원칙 128) — 칩 이름이 「갚는 대출」 → **「갚고 있는 대출」**이 됐습니다.
     ⏹ 「갚는 대출」은 누가 갚는지가 없는 잘린 말이었고, 이 앱이 이미 세 곳에서 쓰는
       「갚고 있는 대출」로 맞췄습니다(원칙 58).
     ⚠ 잠글 사실은 그대로 — **조건칩이 그 대출을 이름으로 말하는가.** 검사를 안 지웠습니다. */
  tt('조건 칩에 갚고 있는 대출이 노출', /갚고 있는 대출<\/u>/.test(UI));
  ...
  /* 🔴 v26.0 재타겟(원칙 128 · 48) — 문장이 어순 규칙으로 바뀌었습니다
     (「갚는 대출을 **반영한** 값이에요」 → 「**넣어서 계산한** 값이에요」 · 한자어를 동사로).
     ⏹ 전에는 **소스 문자열을 통째로** 잡고 있어서, v24.18이 경고한 그 형태였습니다 —
       정당한 문구 개선이 검사에 막힙니다(원칙 48).
     → 잠글 사실만 남깁니다 : **소득 갈래가 부채 유무로 갈리고, 양쪽 다 그 대출을 말하는가.** */
  tt('한도 안내가 부채 입력 여부를 반영', (()=>{
     const m = UI.match(/소득이 한도를 정했어요[\s\S]*?`;/);
     if(!m) return false;
     const t = m[0];
     return /S\.debtMonthly \?/.test(t)            /* 갈래가 있는가 */
         && /갚는 대출을 넣어서 계산한 값이에요/.test(t)   /* 넣었을 때 */
         && /갚고 있는 대출은 02단계에서 넣을 수 있어요/.test(t); /* 안 넣었을 때 */
  })());
  /* 🔴 v26.0 신설 — **이 자리가 다시 지시문으로 돌아가지 않았는가.**
     ⏹ 전 : 「갚고 있는 대출이 있다면 02단계에서 **넣어 주세요.**」 — RULES 말투가 금지하는
       지시문이었고, 5-2가 잰 **390px 고아 줄**이기도 했습니다. 한 동작이 둘을 고쳤습니다.
     ⚠ 「넣을 수 있어요」는 **가능**이지 지시가 아닙니다. 그 경계를 잠급니다. */
  tt('한도 안내가 지시문으로 돌아가지 않았다', (()=>{
     const m = UI.match(/function bindingTip\([\s\S]*?\n\}/);
     return !!m && !/주세요|하세요|하십시오|바랍니다/.test(m[0].replace(/\/\*[\s\S]*?\*\//g,''));
  })());
  ...
  tt('긴 단위를 먼저 잡는다 (억원이 억+원으로 안 쪼개짐)',
     /\(억원\|만원\|억\|원\)/.test(UI));
  /* 🔴 v26.2 재타겟(원칙 128 · 182) — 겨눔이 `class="u"`를 **정확히** 보고 있었습니다.
     v26.2가 단위를 `class="u ue"` · `class="u um"` 두 단으로 가르면서, 옛 겨눔은
     **붙은 단위 span 둘을 보고도 초록**이 됩니다 — 잡으려던 모양이 그대로 지나갑니다.
     ⚠ 잠글 사실은 그대로입니다 : **단위 span이 둘 연달아 열리지 않는다.**
       클래스 목록이 아니라 **`.u`로 시작하는 단위 span인가**를 봅니다. */
  tt('단위 span을 인접해 두 번 열지 않는다',
     !/<\/i>\s*<i class="u[ "]/.test(UI), (UI.match(/<\/i>\s*<i class="u[^"]*"/g)||[]).join(' '));
  /* 🔴 v26.2 신설 — **단위 두 단이 실제로 두 값인가.**
     ⚠ 클래스만 갈라 놓고 크기를 같게 두면 마크업은 둘인데 **화면은 그대로**입니다
       (원칙 149 — 잴 것은 이름이 아니라 보이는 사실). 억 > 기준 > 만원 순서를 잽니다.
     ⚠ 값을 잠그지 않습니다 — **부등호만** 봅니다. 다음 판이 스케일을 옮겨도 안 거짓말합니다. */
  tt('단위가 억 > 기준 > 만원 세 단이다 (v26.2)', (()=>{
     const css = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'');
     const em = re => Number(((css.match(re)||[''])[0].match(/font-size:([\d.]+)em/)||[])[1]);
     const base = em(/\n\.u\{[^}]*\}/);
     const up = em(/\n\.rhead-amount \.ue\{[^}]*\}/), dn = em(/\n\.rhead-amount \.um\{[^}]*\}/);
     return up > base && base > dn && dn > 0;
  })(), (()=>{ const css = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'');
     const em = re => ((css.match(re)||[''])[0].match(/font-size:[\d.]+em/)||['?'])[0];
     return 'ue ' + em(/\n\.rhead-amount \.ue\{[^}]*\}/) + ' / u ' + em(/\n\.u\{[^}]*\}/)
          + ' / um ' + em(/\n\.rhead-amount \.um\{[^}]*\}/); })());
  /* 🔴 v26.2 신설 — **두 단이 히어로 금액 밖으로 안 샌다.**
     ⏹ 전역으로 걸었더니 원리금 타일의 「만원」이 **10.88px**이 됐습니다 — 이 앱의 제일 작은
       단(--t8 12px)보다 작고, 그 값엔 억이 없어 대비를 만들 상대도 없습니다(실측 · 390px).
     ⚠ 잠글 사실은 「.rhead-amount 안에서만 산다」입니다. 겨눔 없는 `.ue{` · `.um{`이
       스타일시트에 생기면 빨간불입니다. */
  tt('단위 두 단이 히어로 금액 밖으로 안 샌다 (v26.2)', (()=>{
     const css = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'');
     return !/\n\.ue\{/.test(css) && !/\n\.um\{/.test(css)
         && /\n\.rhead-amount \.ue\{/.test(css) && /\n\.rhead-amount \.um\{/.test(css);
  })());
  /* 🔴 v26.2 신설 — **`억원`이 큰 쪽에 남는가.** man이 0이면 「18억원」 한 덩어리가 나옵니다.
     그 자리의 「원」은 잔돈 단위가 아니라 화폐 꼬리라 **작은 쪽으로 내리면 안 됩니다.** */
  tt('억원이 큰 단위 쪽이다 (v26.2)',
     /m === '억' \|\| m === '억원' \? 'ue' : 'um'/.test(UI));
  ...
  /* 🔴 v26.2 재타겟(원칙 128) — **답 덩어리에 줄이 하나 더 붙었습니다**(히어로 이유 줄).
     덩어리는 이제 라벨 → 금액 → 이유 줄이고, 24px은 **그 덩어리 아래**에 있어야 합니다.
     ⏹ 금액 아래에 24를 그대로 두면 이유 줄이 24 · 24 사이에 떠서 **위아래 어느 쪽에도
       안 붙습니다** — v25.49가 막으려던 「어디까지가 답인지 안 갈린다」가 그대로 돌아옵니다.
     ⚠ 검사를 지우지 않았습니다. 잴 사실은 그대로이고 **자리만 옮겼습니다** —
       ① 답 덩어리 아래 ≥ 20px ② 금액과 이유 줄은 라벨 → 금액과 **같은 8px**(한 덩어리). */
  tt('히어로 답 덩어리 아래에 여백이 있다 (v25.49 · v26.2 재타겟)', (()=>{
     const css = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'');
     const amt  = (css.match(/\.rhead-amount\{[^}]*\}/)||[''])[0];
     const note = (css.match(/\.rhead-note\{[^}]*\}/)||[''])[0];
     const a = amt.match(/margin:8px 0 (\d+)px/);
     const n = note.match(/margin:0 0 (\d+)px/);
     return !!a && Number(a[1]) === 8 && !!n && Number(n[1]) >= 20;
  })(), (()=>{ const css = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'');
     const mg = re => ((css.match(re)||[''])[0].match(/margin:[^;}]*/)||['?'])[0];
     return '금액 ' + mg(/\.rhead-amount\{[^}]*\}/) + ' / 이유 줄 ' + mg(/\.rhead-note\{[^}]*\}/); })());

  /* 🔴 v26.2 신설 — **히어로 이유 줄이 그 자리에 있는가.**
     ⚠ 잠글 것은 문장 전체가 아니라 **셋**입니다(원칙 48 · 128) —
       ① 금액과 조건 요약 **사이**에 있는가(자리)
       ② 못 쓰기로 한 문장(「세금과 수수료」 열거)으로 돌아가지 않았는가
          — 이사·가전을 켜면 거짓이 되는 문장입니다(0장 ⓪-3 ①)
       ③ 선택자 이름이 `.rhead-why`가 아닌가 — 그 이름은 v25.20의 알약 폐기를 지키는 자리입니다. */
  tt('히어로 이유 줄이 금액과 조건 요약 사이에 있다 (v26.2)', (()=>{
     const bare = fs.readFileSync(FILE,'utf8').replace(/<!--[\s\S]*?-->/g,'');
     const head = (bare.match(/<header class="rhead">[\s\S]*?<\/header>/)||[''])[0];
     const at = x => head.indexOf(x);
     return at('class="rhead-note"') > at('id="heroAmount"')
         && at('class="rhead-note"') < at('id="condBar"')
         && /<p class="rhead-note">집값에, 집 살 때 같이 나가는 돈까지 더했어요<\/p>/.test(head);
  })());
  /* 🔴 v26.2 신설 — **항목을 열거하는 문장으로 돌아가지 않았는가.**
     「세금과 수수료까지」는 부대비용의 **일부**만 셉니다. 이사·입주청소 · 가전·가구 ·
     인테리어는 세금도 수수료도 아닌데 같은 총액에 들어갑니다(원칙 39 — 화면이 실제보다
     좁은 범위를 말하면 그건 틀린 말입니다). */
  tt('히어로 이유 줄이 부대비용 항목을 열거하지 않는다 (v26.2)', (()=>{
     const bare = fs.readFileSync(FILE,'utf8').replace(/<!--[\s\S]*?-->/g,'');
     const line = (bare.match(/<p class="rhead-note">([^<]*)<\/p>/)||['',''])[1];
     return !!line && !/세금|수수료|취득세|중개보수/.test(line);
  })());
  ...
  /* 🔴 v25.61 신설 — **「결과에서 왔다」가 새로고침을 견딘다.**
     ⏹ 오너 실기 지적 : 조건칩으로 퍼널에 들어간 뒤 되돌아갈 길이 없어집니다.
       재현 — 칩 누른 직후 `fromResult=true`·「결과 보기」 보임 → **새로고침** →
       `fromResult=false`·**버튼 사라짐**·step 0. **사용자가 퍼널에 갇힙니다.**
     ⏹ 원인은 `DRAFT_KEYS`에 `fromResult`가 **없던 것**입니다. `onResult`·`lockedPrice`는
       v23.27에 들어왔는데 이것만 빠졌습니다 — 같이 만들어진 상태가 같이 저장되는지
       확인하지 않은 자리입니다.
     🔴 **왜 기존 검사가 못 잡았나** — `flow50`은 한 세션 안에서만 「칩 → 퍼널 → 결과」를 봅니다.
       **새로고침을 안 겁니다.** 저장·복원 경로를 통째로 건너뛰니 지나갔습니다.
       실기에서는 사용자가 새로고침을 안 눌러도, 모바일 브라우저가 탭을 뒤로 보내면
       페이지를 다시 읽어 **같은 일이 납니다.** */
  /* 🔴 v25.67 — 저장 목록 둘을 **파싱해서** 씁니다. 문자열로 「'x' 가 들어 있는가」를
     보면 주석에 그 이름이 있기만 해도 참이 되고, 순서에도 걸립니다(원칙 182). */
  const listOf = name => {
    const m = UI.match(new RegExp('const ' + name + ' = \\[[\\s\\S]*?\\];'));
    if(!m) return [];
    /* 주석을 떼고 셉니다 — **기록은 목록이 아닙니다**(v25.66이 여백 스케일에서 배운 자리). */
    return [...m[0].replace(/\/\*[\s\S]*?\*\//g,'').matchAll(/'([A-Za-z]\w*)'/g)].map(x => x[1]);
  };
  const draftKeys = () => listOf('DRAFT_KEYS');
  const draftSkip = () => listOf('DRAFT_SKIP');
  /* 🔴 v25.67 — **겨눔을 옮겼습니다.** 전에는 `/'fromResult'\]/` — 「목록의 **마지막**이고
     바로 `]`가 온다」를 보고 있었습니다. v25.67이 뒤에 부대비용 넷을 더하자 **터졌습니다.**
     ⏹ 잠근 **사실**은 「`fromResult`가 저장 목록에 **있다**」이지 「마지막이다」가 아닙니다.
       사실은 그대로 두고 겨눔만 옮깁니다(원칙 128 · 175).
     ⏹ v25.65의 `.card-title`와 **같은 종류**입니다 — 「우연히 맞고 있던 겨눔」(원칙 182).
       목록에 무엇이 더 붙어도 이제 안 깨집니다. */
  tt('결과에서 왔다는 사실이 새로고침을 견딘다 (v25.61)',
     draftKeys().includes('fromResult'));
  /* ⚠ 되읽을 때 **형을 맞추는지**까지 봅니다 — localStorage는 문자열을 돌려주므로
     `"false"`가 참이 됩니다. 위 `onResult`와 같은 처리여야 합니다(원칙 58).
     ⚠ 그리고 **결과 화면으로 복원되면 끕니다** — 안 끄면 다음에 퍼널로 들어갈 때
       오지 않은 곳에서 온 것처럼 보입니다(원칙 54). */
  tt('되읽은 fromResult가 형을 맞춘다 (v25.61)',
     /S\.fromResult = !!S\.fromResult && !S\.onResult;/.test(UI));
  /* 🔴 **저장 목록에 빠진 상태가 또 없는지** 봅니다. 화면 흐름을 정하는 상태는
     전부 저장돼야 하고, 하나라도 빠지면 새로고침에서 조용히 어긋납니다.
     ⚠ 이 검사는 「목록에 있는가」만 봅니다 — **동작은 사보타주가 봅니다.** */
  tt('흐름 상태가 전부 저장 목록에 있다 (v25.61)', (()=>{
     const k = (UI.match(/const DRAFT_KEYS = \[[\s\S]*?\];/)||[''])[0];
     /* 🔴 v25.62 — `loanCap`을 목록에 더했습니다.
        ⚠ **사보타주가 잡았습니다.** v25.62가 새 상태를 만들고도 이 목록에 안 넣어서,
          `'loanCap',`을 지워도 1,038개가 전부 초록이었습니다.
          **원칙 176을 세운 판이 바로 다음 판에서 그 원칙을 어겼습니다** —
          원칙은 적는다고 지켜지지 않고, **잠금으로 걸어야** 지켜집니다. */
     return ['step','onResult','lockedPrice','fromResult','mciCovered','loanCap']
       .every(x => k.includes("'"+x+"'"));
  })());
  /* 🔴 v25.67 신설 — **부대비용 네 줄이 새로고침을 견딘다.**
     ⏹ `tools/restore.mjs` H가 잡았습니다(390px 실측) : 「이사 · 입주청소」와 「가전 · 가구」를
       켜고 새로고침하면 **준비할 현금 15억 675만원 → 15억원**(−675만원). 켠 비용이 사라져
       필요한 현금이 실제보다 **적어 보입니다** — 🔴 유리한 쪽 오차입니다(원칙 28).
     ⏹ **v24.7과 같은 종류**입니다. 그 판이 인테리어 셋을 목록에 넣으면서
       **부대비용 넷은 안 넣었습니다** — 같은 화면의 같은 종류인데 한쪽만 실렸습니다.
     ⚠ 검사 1,047개 · selfcheck 30개 · flow50이 전부 초록인 채로 두 해를 지났습니다.
       재는 장치가 없는 자리는 **잠금도 못 만듭니다**(감사 6-4). */
  tt('부대비용 네 줄이 새로고침을 견딘다 (v25.67)',
     ['broker','legal','move','appl'].every(x => draftKeys().includes(x)));
  /* 🔴 v25.67 신설 — **「빠뜨린 것」과 「일부러 뺀 것」을 가르는 목록이 있다.**
     ⏹ v25.61의 `fromResult`가 한 판 동안 조용했던 이유가 이것입니다 — 목록에 없다는
       사실만으로는 **실수인지 판단인지 알 수 없었습니다.** 양쪽 다 목록으로 두면
       `S`의 새 키가 어느 쪽에도 없을 때 게이트가 즉시 빨간불을 냅니다(원칙 149 · 176).
     ⚠ 여기서는 **목록이 있는가 · 겹치지 않는가**만 봅니다.
       「`S`의 키가 전부 분류돼 있는가」는 **렌더로만** 셀 수 있어 `tools/restore.mjs` ④ 몫입니다.
       (`S`는 함수 안에서도 키가 붙을 수 있어 소스 파싱으로는 못 셉니다 — 원칙 144.) */
  tt('저장 안 하는 키의 목록이 따로 있다 (v25.67)', draftSkip().length > 0);
  /* 겹치면 **어느 쪽이 참인지 모릅니다** — 한 사실을 두 곳에서 다르게 말하는 자리입니다(원칙 58).
     🔴 처음에 `겹침.length === 0 || 겹침.join(' · ')`로 적었다가 **사보타주 67 ㉦이 뚫었습니다** —
       실패할 때 **참인 문자열**이 나가서 늘 초록이었습니다. `tt`의 둘째 인자는 판정이고
       셋째가 설명입니다. 자리를 헷갈리면 잠금이 조용히 죽습니다(원칙 173). */
  tt('두 저장 목록이 안 겹친다 (v25.67)',
     draftKeys().filter(x => draftSkip().includes(x)).length === 0,
     draftKeys().filter(x => draftSkip().includes(x)).join(' · ') || '겹침 0');
  /* ⚠ 되읽을 때 **형을 맞추는지**까지 봅니다 — `loanCap`·`onResult`와 같은 처리여야 합니다(원칙 58). */
  /* 🔴 잠그는 것은 「검사한다」가 아니라 **「형이 아니면 기본값으로 되돌린다」**입니다.
     처음에 `on: !!o.on`이 있는지만 봤다가 **사보타주 67 ㉣이 뚫었습니다** —
     되돌리는 줄만 지워도 그 문자열은 그대로 남아 있었습니다(원칙 181 · 「없음」은 첫 곳으로 부족합니다). */
  tt('되읽은 부대비용이 형을 맞춘다 (v25.67)',
     /typeof o !== 'object'\)\{ S\[k\] = \{ on:d\.on, v:d\.v \}; return; \}/.test(UI)
     && /on: !!o\.on/.test(UI) && /Number\.isFinite\(v\) && v >= 0/.test(UI));
  /* 🔴 **중개보수의 `null`(상한요율 자동)이 살아남는가.**
     `Number(null)`은 **0**입니다. 그냥 숫자로 밀면 「자동」이 「0만원」이 되어
     부대비용이 통째로 빠집니다 — 이것도 유리한 쪽입니다(원칙 28).
     ⏹ 이 한 줄이 없으면 위 H가 고쳐진 자리에서 **더 큰 구멍**이 열립니다. */
  tt('중개보수의 「자동」이 새로고침을 견딘다 (v25.67)',
     /\(o\.v == null\) \? d\.v/.test(UI));
  /* 🔴 기본값을 **손으로 다시 적지 않는가**(원칙 84).
     ⏹ v24.7이 정확히 이것으로 났습니다 — `loadDraft` 안의 `[0,100,150,220]`이
       실제 `INTERIOR`(`[0,80,150,250]`)와 달랐고, 그 **둘째 정의**가 조용히 이겼습니다.
     ⚠ 그래서 `COST_DEFAULT`는 리터럴이 아니라 **`S`에서 떠 온 것**이어야 합니다. */
  tt('부대비용 기본값을 두 번 적지 않는다 (v25.67)',
     /const COST_DEFAULT = Object\.fromEntries\(COST_KEYS\.map\(k => \[k, \{ on: S\[k\]\.on, v: S\[k\]\.v \}\]\)\);/.test(UI));
  /* 🔴 v25.67 신설 — **주석이 실물과 갈리지 않는가**(원칙 47 · 184).
     ⏹ `S` 정의의 v25.14 주석이 `fromResult`를 「초안에 저장하지 않습니다」라고 적어 뒀는데
       **v25.61이 저장 목록에 넣었습니다.** 두 판 동안 주석만 옛말이었고,
       그 주석대로 「고치면」 오너가 실기에서 지적한 버그가 **그대로 되살아납니다.**
     ⚠ 잠그는 것은 문장이 아니라 **모순**입니다 — 목록에 있으면서 「저장 안 한다」고
       적힌 상태가 되지 않게. `.legend` 주석이 죽은 규칙을 살려 두던 것과 같은 계열입니다. */
  tt('저장하는 값을 「저장 안 한다」고 적어 두지 않는다 (v25.67)', (()=>{
     /* `fromResult:false,` 바로 **앞**의 주석 덩어리만 봅니다 — 그 값이 어떤 값인지
        설명하는 자리가 거기입니다. 소스 전체에서 문구를 찾으면 다른 키를 설명하는
        정당한 문장까지 잡습니다(원칙 147 — 어느 그릇에 담겨 있는지 확인하고 잠급니다). */
     const at = UI.indexOf('fromResult:false,');
     if(at < 0) return false;
     const 앞 = UI.slice(Math.max(0, at - 1800), at);
     return !(/초안에 저장하지 않습니다/.test(앞) && draftKeys().includes('fromResult'));
  })());
  tt('가진 돈 초과 안내가 있다', /class="overnote"/.test(UI));
  /* 🔴 v25.57 신설 — **방공제를 끄고 켤 수 있다.**
     ⏹ 엔진에는 스위치가 처음부터 있었고(`roomDeduction`의 `ctx.mciCovered`),
       `ctx()`가 `false`를 **상수로** 박아 화면에서 못 닿았습니다. 지침의
       「기능을 추가하면 그 입력칸에 사용자가 실제로 도달 가능한가」가 걸리던 자리입니다.
     ⚠ **상수로 되돌아가지 않는가**를 봅니다 — 그러면 체크박스는 남고 값만 안 먹습니다.
     ⚠ 기본은 **끔**(방공제 적용)입니다. 켜면 한도가 늘어 결과가 유리해지므로
       유리한 쪽을 기본값으로 두지 않습니다(지침 · 원칙 28). */
  tt('방공제 스위치가 상태와 이어져 있다 (v25.57)',
     /mciCovered:S\.mciCovered/.test(UI) && !/mciCovered:false,\s*roomDeductAmt/.test(UI)
     && /mciCovered:false/.test(UI.match(/const S = \{[\s\S]*?\n\};/)||[''][0] || UI));
  /* ⚠ 마크업은 `UI`(스크립트)가 아니라 **전체 소스**에서 봅니다 — 처음에 UI로 썼다가
     빨간불이 났습니다. 「어느 그릇에 담겨 있는가」를 확인하고 잠급니다(원칙 147). */
  /* 🔴 v25.62 신설 — **주담대 상한을 직접 낮출 수 있다**(오너 지시 — 「국민은행은 15억 이하
     3억으로 정부 상한보다 낮다」 · 「6억 나와도 4억만 받고 싶을 수 있다」).
     ⏹ 엔진은 처음부터 준비돼 있었습니다 — `manualLoanCap`이 상한을 덮고 `binding='manual'`까지
       세웁니다. `ctx()`가 **0을 박아** 화면에서 못 닿았을 뿐입니다(방공제 v25.57과 같은 모양).
     ⚠ **상수로 되돌아가지 않는가**를 봅니다 — 그러면 칸은 남고 값만 안 먹습니다. */
  /* 🔴 v25.62 — **넣은 상한이 새로고침을 견딥니다**(원칙 176 ①②).
     ⚠ 되읽을 때 **형을 맞추는지**까지 봅니다 — localStorage는 문자열을 돌려줍니다. */
  tt('주담대 상한이 새로고침을 견딘다 (v25.62)',
     /const lc = Number\(S\.loanCap\);/.test(UI)
     && /S\.loanCap = \(Number\.isFinite\(lc\) && lc > 0\)/.test(UI));
  tt('주담대 상한이 상태와 이어져 있다 (v25.62)',
     /manualLoanCap:man\(S\.loanCap\)\|\|0/.test(UI) && !/manualLoanCap:0,/.test(UI));
  /* ═══ v25.68 — 도달 가능성 ═══════════════════════════════════
     🔴 같은 결함이 **세 번** 났고 셋 다 모양이 같습니다 — 엔진에는 스위치가 있는데
       `ctx()`가 **상수를 박아** 화면에서 아무도 못 닿는 것.
         v25.57  mciCovered      → 방공제
         v25.62  manualLoanCap   → 사용자가 정한 상한
         지금    loanChoice      → POLICY_DEFS 95줄 (오너 결정 : 하지 않습니다)
       셋 다 **값이 틀린 게 아니라 안 움직인 것**이라 검사도 selfcheck도 할 말이 없었습니다.
     → `CTX_FIXED`가 「고정이다」와 「빠뜨렸다」를 가릅니다(원칙 149).
     ⚠ 여기서는 **소스로 확인되는 것만** 봅니다. 「화면에서 정말 안 움직이는가」는
       렌더로만 셀 수 있어 `tools/reach.mjs` ②·③ 몫입니다(원칙 144). */
  const ctxBody = ((UI.match(/function ctx\(\)\{[\s\S]*?\n\}/)||[''])[0]).replace(/\/\*[\s\S]*?\*\//g,'');
  /* ⚠ 키는 `{`나 `,` 바로 뒤여야 합니다 — 안 그러면 삼항의 뒷가지(`… ? S.itCustom : 0`)를
     「상수」로 셉니다. 처음에 그렇게 셌다가 없는 상수 셋이 잡혔습니다(원칙 84). */
  const ctxLit = [...ctxBody.matchAll(/[{,]\s*(\w+)\s*:\s*(-?\d+(?:\.\d+)?|'[^']*'|"[^"]*"|true|false|null)\s*(?=[,}])/g)].map(x=>x[1]);
  const ctxFixed = (()=>{ const m = UI.match(/const CTX_FIXED = \[[\s\S]*?\n\];/);
    return m ? [...m[0].replace(/\/\*[\s\S]*?\*\//g,'').matchAll(/'([A-Za-z]\w*)'/g)].map(x=>x[1]) : []; })();
  tt('고정으로 넘기는 값의 목록이 있다 (v25.68)', ctxFixed.length > 0);
  /* 🔴 **이 한 줄이 이 판의 본체입니다.** `ctx()`에 새 상수를 넣으면 여기서 즉시 빨간불입니다 —
     v25.57 · v25.62가 각각 여러 판씩 조용했던 그 자리입니다. */
  tt('ctx()의 상수가 전부 근거와 함께 적혀 있다 (v25.68)',
     ctxLit.every(k => ctxFixed.includes(k)),
     ctxLit.filter(k => !ctxFixed.includes(k)).join(' · ') || '빠진 것 0');
  tt('고정 목록에 ctx()에 없는 이름이 없다 (v25.68)',
     ctxFixed.every(k => new RegExp('[{,]\\s*'+k+'\\s*:').test(ctxBody)),
     ctxFixed.filter(k => !new RegExp('[{,]\\s*'+k+'\\s*:').test(ctxBody)).join(' · ') || '죽은 이름 0');
  /* 🔴 **이어진 값을 「고정」이라 적어 두지 않는다.** 남겨 두면 주석이 죽은 사실을 지키고,
     다음 사람은 그 값에 화면이 안 닿는 줄 압니다(원칙 47 · `.legend`가 그랬던 자리). */
  tt('상태와 이어진 값이 고정 목록에 없다 (v25.68)',
     !['mciCovered','manualLoanCap','noLoan','income','pyeong'].some(k => ctxFixed.includes(k)));
  /* 근거가 **주석**으로 있어야 합니다. 이름만 늘어놓으면 다음 사람이 「왜 고정인가」를 못 읽고,
     그러면 이 목록은 「고쳐도 되는 것」과 「고치면 안 되는 것」을 못 가릅니다.
     ⚠ 근거를 **문자열**로 두면 `test.js`가 그것을 화면 문구로 셉니다(이모지 · 「~습니다」체) —
       처음에 객체로 적었다가 검사 둘이 빨간불을 냈습니다(원칙 147). */
  tt('고정 목록의 근거가 주석으로 있다 (v25.68)', (()=>{
     const m = UI.match(/const CTX_FIXED = \[[\s\S]*?\n\];/);
     if(!m) return false;
     return (m[0].match(/\/\*/g)||[]).length >= ctxFixed.length;
  })());
  /* 🔴 **낮추는 쪽으로만 움직입니다.** `finish()`가 `amount > ctx.manualLoanCap`일 때만 덮으므로
     실제 한도보다 큰 값을 넣어도 한도가 **안 늘어납니다** — 유리한 방향으로 못 갑니다(원칙 28).
     ⏹ 실측 : 9억을 넣어도 정부 상한(3억 4,500만)이 그대로 걸립니다. */
  tt('주담대 상한이 한도를 늘리지 못한다 (v25.62)',
     /amount > ctx\.manualLoanCap/.test(fs.readFileSync(FILE,'utf8')));
  /* 🔴 **한도 이름표에 `manual`이 있어야 합니다.** 엔진은 v24부터 `binding='manual'`을 세웠는데
     `LNAME`에 이름이 없어서, 그 길로 가면 화면이 **원시 키 `manual`을 그대로 찍었습니다**
     (`LNAME[k]||k`). 아무도 그 길을 못 갔기 때문에 안 드러났을 뿐입니다. */
  tt('직접 정한 상한에 이름이 있다 (v25.62)',
     /manual:'직접 정한 상한'/.test(UI));
  /* 🔴 **켰으면 걸렸는지 아닌지를 말합니다**(원칙 54). 넣었는데 아무 말도 없으면
     사용자는 「내 입력이 먹었나」를 모릅니다. 두 경우를 나눠 적습니다. */
  tt('상한이 걸렸는지 화면이 말한다 (v25.62)',
     /넣으신 상한이 걸렸어요/.test(UI) && /넣으신 상한은 안 걸렸어요/.test(UI));
  /* 🔴 **켜면 집값을 다시 정합니다.** 상한은 비용이 아니라 **한도**라, 낮추면 살 수 있는 집이
     작아지는 것이 이 앱이 답하는 질문입니다(방공제 v25.57과 같은 갈래 · 원칙 171). */
  tt('주담대 상한을 바꾸면 집값을 다시 정한다 (v25.62)', (()=>{
     const f = (UI.match(/const w = \$\('swCap'\)[\s\S]*?i\.onclick/)||[''])[0];
     return /S\.lockedPrice = null; showResult\(true\);/.test(f)
         && /S\.loanCap = w\.checked \? 0 : null;/.test(f);
  })());
  /* ⚠ **`.subtoggle` 문법을 씁니다** — 바로 위 MCI 줄과 형제입니다(원칙 58 · 지침 6-22).
     🔴 `.costrow`를 쓰면 안 됩니다 : 이 서랍 전체를 보는 검사의 정규식이 안쪽 두 겹 닫힘에서
       잘려 `limitNotes`를 못 찾습니다. v25.62가 그걸로 두 번 빨간불을 봤습니다. */
  tt('주담대 상한 줄이 화면에 있다 (v25.62)', (()=>{
     const full = fs.readFileSync(FILE,'utf8');
     return /id="swCap"/.test(full) && /class="subtoggle" id="capRow"/.test(full)
         && !/class="costrow" id="capRow"/.test(full);
  })());
  tt('방공제 스위치가 화면에 있다 (v25.57)',
     (()=>{ const full = fs.readFileSync(FILE,'utf8');
        return /id="swMci"/.test(full) && /class="subtoggle" id="mciRow"/.test(full); })());
  /* 🔴 **집값을 다시 정합니다**(오너 결정 — 「방공제 MCI 가입해서 대출이 더 나오면
     살 수 있는 집이 달라지는데」). `S.lockedPrice = null`이 없으면 집값이 고정된 채
     **「같은 집을 현금 덜 내고 산다」**가 되고, v25.55의 「준비할 현금 = 입력한 현금」이 깨집니다.
     ⏹ 실측 : 고정이면 현금 5억 넣은 사람에게 「준비할 현금 4억 4,500만원」이 뜹니다.
     ⚠ v23.9의 「집값은 한 번만 정한다」와 안 부딪힙니다 — 그건 **비용**(인테리어 · 이사비)이
       집값을 흔드는 것을 막는 규칙이고, 방공제는 비용이 아니라 **한도**입니다(원칙 171). */
  tt('방공제를 켜면 집값을 다시 정한다 (v25.57)',
     /S\.mciCovered = i\.checked; S\.lockedPrice = null; showResult\(true\)/.test(UI));
  /* 🔴 **켰을 때도 이유를 말합니다**(원칙 54 — 새 상태를 만들면 거짓말하는 화면을 훑습니다).
     껐을 때만 뜨게 두면 스위치를 켠 순간 설명이 사라지고 체크박스만 남습니다. */
  tt('방공제 각주가 켰을 때도 뜬다 (v25.57)',
     /roomTip'\)\.hidden = !\(c\.roomCut>0 \|\| mciOn\)/.test(UI)
     && /MCI · MCG에 가입하면 은행이 방공제를 떼지 않아요/.test(UI));
  /* 🔴 v25.57 신설 — **현금이 남을 때도 말합니다.**
     ⏹ 전에는 모자랄 때만 말하고 남을 때는 침묵이었습니다. **15억 규제 절벽**에서
       집값이 멈춰 현금이 남는데(대출 상한 6억→4억), 화면이 그걸 안 말했습니다.
     ⏹ 실측 1,896케이스 중 **90건(4.7%)**에서 남고, **최대 1억 9,030만원**입니다.
     ⚠ 같은 상자(`.overnote`)를 씁니다 — 한 자리의 두 방향이고 동시에 뜰 수 없습니다.
     ⚠ 문턱이 **양쪽 다 1만원**인지 봅니다. 한쪽만 좁으면 같은 크기의 차이가
       한 방향에서만 보고됩니다(원칙 91). */
  ...
  /* 🔴 v25.57 — **양쪽 문턱이 같은 값인가.**
     ⚠ 🔴 이 잠금은 **사보타주가 찾아냈습니다.** 위 검사는 `over < -10000`이 **있는지**만 봐서,
       ...
       다른 사실이고, 제가 앞의 것만 잠갔습니다(원칙 170 — 잴 축을 다 적어라).
     ⏹ 문턱이 갈리면 **같은 크기의 차이가 한 방향에서만 보고됩니다** —
       1억이 모자라면 말하고 1억이 남으면 침묵하는 화면이 됩니다(원칙 91).
     ⚠ 두 숫자를 **뽑아서 절댓값을 비교**합니다. 리터럴을 문자열로 맞추면
       공백 하나에 흔들립니다(원칙 48 — 표현이 아니라 의도). */
  tt('가진 돈 안내의 문턱이 양쪽 같다 (v25.57)', (()=>{
     const up = (UI.match(/over >\s*(\d+)/)||[])[1];
     const dn = (UI.match(/over <\s*-(\d+)/)||[])[1];
     return !!up && !!dn && Number(up) === Number(dn);
  })());
  ...
  /* 🔴 v25.31 — **이 검사가 주석을 보고 있었습니다**(원칙 152).
     v25.31이 범례 주석에 실측 표를 적으며 화살표 한 글자를 쓰자 **화면은 그대로인데 빨간불**이
     났습니다. 이 검사가 잠그려던 사실은 「**화면 마크업**에 화살표가 없다」인데, `UI`는
     주석까지 들고 있는 화면 코드 전체입니다.
     → 마크업을 보는 검사이므로 **블록 주석과 HTML 주석만** 걷고 봅니다.
     ⚠ 줄 주석(`//`)은 안 걷습니다 — URL에도 있습니다(원칙 152의 반대편). */
  const UI_NOCMT = UI.replace(/\/\*[\s\S]*?\*\//g, '').replace(/<!--[\s\S]*?-->/g, '');
  tt('되돌아가기가 텍스트만이다',
     /id="reeditBtn">이전 단계</.test(UI) && !/←/.test(UI_NOCMT));
  ...
  /* 🔴 v25.52 — **「지우기」 손가락 면**(인수인계 0장 ③㉯ · 원칙 112 · 123).
     실측 390px에서 **33 × 22px**이었습니다 — 세로만이 아니라 **가로도** 모자랐습니다.
     ⚠ 그래서 이 자리만 **두 축을 다** 잽니다. 집안의 다른 손가락 면(.cond · .chip.sm · .subtoggle)은
       가로가 이미 44를 넘어 세로만 폈고, 여기는 그 전제가 안 섭니다(원칙 147 — 같은 이름의
       결함이라고 같은 처방이 아닙니다). */
  tt('「지우기」 손가락 면이 --tap이다 (가로 · 세로 둘 다 · v25.52)',
     /\.clearbtn\{[^}]*position:relative/.test(css2)
     && /\.clearbtn::after\{[^}]*width:var\(--tap\)/.test(css2)
     && /\.clearbtn::after\{[^}]*height:var\(--tap\)/.test(css2));
  /* 🔴 **아래로 못 폅니다.** 라벨 옆 「지우기」는 아래 여유가 **8px**뿐이라(실측 390px)
     세로 가운데로 얹으면 투명 면이 금액 입력칸 윗머리를 약 3px 덮습니다 —
     칸을 누르려다 값이 지워집니다. 바닥에 못박고 **위로만** 폅니다(위쪽 여유 138px · 30px).
     ⚠ `top:50%`나 `translateY`가 들어오면 그 순간 되돌아갑니다. 그것을 잠급니다. */
  tt('「지우기」 손가락 면이 위로만 편다 (아래 입력칸을 안 덮는다 · v25.52)',
     /\.clearbtn::after\{[^}]*bottom:0/.test(css2)
     && !/\.clearbtn::after\{[^}]*translateY/.test(css2)
     && !/\.clearbtn::after\{[^}]*top:50%/.test(css2));
  /* ⚠ 투명 면은 **자리를 먹으면 안 됩니다** — 먹으면 라벨 줄이 44px로 부풉니다. */
  tt('「지우기」 손가락 면이 자리를 안 먹는다 (v25.52)',
     /\.clearbtn::after\{[^}]*position:absolute/.test(css2));
  /* 🔴 「지우기」는 화면에 **둘**입니다(카드 모서리 .only-clear · 라벨 옆 .flabel).
     규격을 둘로 두면 같은 부품이 두 자리에서 다르게 눌립니다(원칙 58) —
     **수식자에 손가락 면을 따로 적지 않는가**를 봅니다. */
  tt('「지우기」 손가락 면 규격이 하나다 (v25.52)',
     !/\.only-clear\s+\.clearbtn::after/.test(css2)
     && !/\.clearbtn\.[\w-]+::after/.test(css2));
  ...
  /* 🔴 v25.40 — 세로선이 있던 자리입니다. 지금 첫 화면 상단은 **한 줄 각인**이고,
     잠글 것은 그 줄의 **규격**입니다 — 크기 · 500 · `--ink-3`.
     ⚠ 색이 `--ink-4`면 앱 배경 위 4.19:1로 미달입니다(원칙 97). 그 사실을 여기서 잠급니다.
     🔴 v25.53 재타겟 — 크기가 **`--t8`(12) → `--t7`(13)**입니다(오너 지시).
       🔴 v25.58 재타겟 — 다시 **`--t7`(13) → `--t4`(17)**입니다(오너 지시 —
         「크기는 영끌계산기와 같은 걸로」). 워드마크와 **같은 칸**이 됐습니다.
         ⚠ 그래서 **굵기(500)와 색(`--ink-3`)이 위계를 혼자 집니다.** 이 검사가 그 둘을
           같이 보는 것이 v25.58부터 더 중요해졌습니다 — 하나라도 워드마크 쪽으로 가면
           이름과 설명이 구분되지 않습니다.
       ⏹ v25.40이 12px을 적은 것은 「12여야 한다」가 아니라 **그때 값을 기록한 것**이었고,
         이 잠금이 지키는 사실은 **「각인이 브랜드 규격 셋을 갖췄는가」**입니다.
         값이 바뀌면 값을 옮깁니다 — 잠금을 지우지 않습니다(원칙 128).
       ⚠ 12px이던 근거(v25.43의 폭)는 **이미 만료됐습니다** — 실측 폭이 257.7 → 102.6px입니다
         (원칙 148 · 166 — 근거를 같이 적고, 근거가 늙으면 다시 잽니다). */
  tt('첫 화면 각인이 브랜드 규격이다 (v25.40 · v25.59 재타겟)',
     /\.brandline\{[^}]*font-size:var\(--t5\)/.test(css2)
...
(function(){
  ...
  /* 🔴 v26.1 — **자에 구멍이 있었습니다.** 겨눔이 「하세요 · 해 주세요 · 하십시오」 셋뿐이라
     **「골라주세요」가 어느 쪽에도 안 걸렸습니다** — `.pane-title` 셋과 `.drawer-t` 하나,
     넷이 여러 판 동안 초록인 채로 지시문이었습니다(원칙 173 · 182 — 우연히 맞고 있던 겨눔).
     ⏹ 이 프로젝트는 지시문을 하나씩 지워 왔는데(v24.18 · 다른 판), 그때마다 **문장을 지웠지
       자를 안 넓혔습니다.** 그래서 같은 꼴이 다른 어미로 들어오면 아무도 안 잡았습니다.
     → **「주세요」로 끝나는 것 전부**를 봅니다. 붙여 쓰든 띄어 쓰든 걸립니다.
     ⚠ 예외는 위에서 이미 걷어냈습니다(「다시 시도해 주세요」 — 실패 뒤 복구 안내).
       새 예외를 더할 때는 **왜 지시가 아닌지**를 거기 적으십시오. */
  const order = [...new Set(s.match(/[가-힣][가-힣 ,·()%]{0,45}(주세요|하세요|하십시오|해보세요)/g) || [])];
...
(() => {
  ...
  const BARE = UI.replace(/\/\*[\s\S]*?\*\//g,'').replace(/\/\/[^\n]*/g,'');
  /* 🔴 v25.37 — **대상을 옮겼습니다**(원칙 128 · 148). 전에는 이름 **전체**를 잠갔고,
     그래서 오너 지시(「주담대가 앞에 들어가야 한다」)가 이 검사에 막혔습니다.
     ⚠ 잠글 사실은 **「확정형이 아니다」** 하나입니다 — 「매달 갚는 돈」(현재형 · 확정)이 아니라
       **「매달 갚을 …」**(미래형 · 가정)인가. 앞에 무엇이 붙는지는 이 검사가 잴 사실이 아닙니다.
     ⚠ 「매달」은 그대로 잠급니다 — 빼면 **총액으로 읽힙니다**(v25.13이 잰 자리 · 원칙 39). */
  /* 🔴 v25.56 재타겟 — **v25.37이 절반만 풀었습니다.**
     ⏹ v25.37은 「앞에 무엇이 붙는지는 잴 사실이 아니다」라고 적고 앞머리(`[^<]*`)를 열었는데,
       **「매달 갚을」과 「원리금」 사이는 붙여 둔 채**였습니다. 그래서 오너 지시
       「매달 갚을 **주담대** 원리금」(수식 순서 교정)이 **다시 이 검사에 막혔습니다.**
     ⚠ 잠글 사실은 여전히 하나입니다 — **확정형(「매달 갚는 돈」)이 아니라 미래형(「매달 갚을 …」)인가.**
       그 사실에 두 낱말의 **인접**은 들어 있지 않습니다. 가운데도 엽니다(원칙 48).
     ⚠ 「매달」은 그대로 잠급니다 — 빼면 **총액으로 읽힙니다**(v25.13이 잰 자리 · 원칙 39). */
  tt('월 상환 타일이 확정형이 아니다 (v25.37 · v25.56 재타겟)',
     /class="tile-k">[^<]*매달 갚을[^<]*원리금</.test(UI0())
     && !/class="tile-k">[^<]*매달 갚는 돈</.test(UI0()));
...
(() => {
  ...
  tt('대차대조표가 「준비할 현금」이다 (v25.51 재타겟)',
     ...
     && !/내가 준비할 현금/.test(BARE));
  /* 🔴 v25.54 재타겟(원칙 128) — 카드 몸통이 대차대조표가 되면서 `mixRow`가 죽고
     `bsIt`가 그 일을 받았습니다. 잠글 사실은 그대로입니다 — **카드가 그 값을 뭐라고 부르는가.**
     ⚠ 화면(`<p class="bs-nm">준비할 현금</p>`)과 **같은 이름**이어야 합니다(원칙 91) —
       한쪽만 고치면 받는 사람이 「다른 값인가」를 묻습니다. */
  tt('공유 카드가 「준비할 현금」이다 (v25.54 재타겟)',
     /bsIt\('준비할 현금'/.test(repBody()));
  /* 🔴 v25.54 신설 — **카드가 좌우 두 쪽을 다 그린다.** v25.51이 화면을 대차대조표로 바꿨을 때
     카드는 머리만 맞추고 몸통이 v25.50에 남아, 총액을 말한 뒤 **마련하는 쪽만** 쪼갰습니다.
     받는 사람은 그 총액 중 얼마가 집값인지 못 봤습니다(실기 캡쳐 2026.08.25).
     ⚠ 네 값이 전부 `heroFigures` 한 곳에서 오는지까지 봅니다 — 좌우 합이 갈리면 표가 깨집니다. */
  tt('공유 카드가 대차대조표 좌우 두 쪽이다 (v25.54)', (()=>{
     const b = repBody();
     /* 🔴 v26.0 재타겟(원칙 128) — 두 머리가 질문형이 됐습니다. 잠글 사실은 그대로 —
        **카드가 좌우 두 쪽을 다 그리고, 화면과 같은 머리를 쓰는가.** */
     return /bs-cap">그럼 이 돈은 어디로 갈까요</.test(b) && /bs-cap">그 돈은 어디서 나올까요</.test(b)
         && /bsIt\('집값'/.test(b) && /bsIt\('부대비용'/.test(b)
         && /bsIt\('주담대'/.test(b)
         && /fig\.pPrice/.test(b) && /fig\.pEtc/.test(b) && /fig\.pCash/.test(b) && /fig\.pLoan/.test(b);
  })());
  /* 🔴 v25.54 신설 — **막대와 색점을 뺐습니다.** 화면에서 뺀 근거가 그대로 걸립니다 —
     비율을 각 줄이 이미 말합니다(원칙 43 · 94). 대신 열마다 4px 막대입니다.
     ⚠ 옛 막대(`.bar` · `.lg .r` 색점)가 되살아나지 않았는가를 같이 봅니다. */
  tt('공유 카드에 자금 구성 막대·색점이 없다 (v25.54)', (()=>{
     const b = repBody();
     return !/<div class="bar">/.test(b) && !/class="lg"/.test(b)
         && !/'d1'|'d2'/.test(b)
         && (b.match(/bs-bar/g)||[]).length >= 1;
  })());
  /* 🔴 v25.54 — **대출이 0이면 그 줄을 안 세웁니다**(원칙 124). 「0억(0%)」은 빈 값이 아니라
     거짓이고, 화면(`bsLoanIt.hidden`)이 이미 그렇게 합니다. 카드도 같아야 합니다(원칙 58). */
  tt('공유 카드가 대출 0에 주담대 줄을 안 세운다 (v25.54)',
     /fig\.L > 0 \? bsIt\('주담대'/.test(repBody()));
  ...
  /* 🔴 v25.54 재타겟(원칙 128) — `mixRow`가 죽고 `bsIt`가 받았습니다. 대상만 옮깁니다.
     ⚠ 🔴 **이 잠금의 이름과 실제가 어긋나 있습니다.** 이름은 「화면과 같은 표기」인데
       화면은 v25.48부터 `formatWon`(정확), 카드는 `eokShort`(축약)입니다 — **다릅니다.**
       그래도 축약을 유지하는 근거는 폭이 아니라 **개인정보**입니다(v24.15 —
       「준비할 현금」은 인테리어를 안 켜면 가진 돈과 같고, 그림은 재전달됩니다).
       ⏹ v25.54 실측 : 카드 열 177px에 「20억 4,900만원」이 136.5px — **폭 근거는 만료**됐습니다.
         남은 근거는 개인정보 하나뿐이고, 그건 **오너가 정할 문제**라 이 판은 안 건드렸습니다.
       ⚠ 정확한 표기로 바꾸기로 하면 이 잠금과 아래 「현금·대출 금액을 찍지 않는다」 둘을
         **같이** 옮겨야 합니다. 하나만 고치면 나머지가 빨간불로 막습니다. */
  /* 🔴 v25.60 재타겟 — 카드가 `eokShort`에서 **눈금 표기 `gridFmt`**로 옮겼습니다.
     ⚠ 잠글 사실은 **함수 이름이 아니라 「축약해서 적는가」**입니다(원칙 175 — 주석이 말한 만큼).
       `eokShort`는 v25.60에서 지웠습니다(부르는 곳 0 · 원칙 47). */
  tt('🔴 공유 카드가 축약 표기를 쓴다 (v25.36 · v25.60 재타겟)',
     /const bsIt = \(name, man, pct\) =>/.test(repBody())
     && /gridFmt\(man\)/.test(repBody())
     && !/eokShort/.test(UI.replace(/\/\*[\s\S]*?\*\//g,'')),
     (repBody().match(/const bsIt[\s\S]*?;\n/)||[''])[0].slice(0,180));
  /* 🔴 v25.60 신설 — **네 값이 같은 눈금이고, 각 열의 둘째가 나머지다.**
     ⏹ 지키는 사실 : **카드에서도 좌우 합이 맞는다.** v25.59까지는 474케이스 중
       **456건(96%)**에서 어긋났고 최대 **1,451만원** 차이였습니다(오너 지적).
       원인은 눈금이 섞인 것 — 1억 이상은 0.1억, 미만은 만원.
     ⚠ 둘째 값을 **따로 반올림하면** 그 순간 합이 깨집니다. `Tg - snap(...)` 형태를 잠급니다.
     ⚠ 흡수 칸은 **부대비용과 주담대**입니다 — 「집값」·「준비할 현금」은 사용자가
       자기 입력·기대와 대조하는 칸이라 오차를 두면 안 됩니다(원칙 174). */
  tt('공유 카드 좌우 합이 눈금에서도 맞는다 (v25.60)', (()=>{
     const b = repBody();
     return /const gridMan = fig\.T >= 10000 \? 1000 : 1/.test(b)
         && /const Tg = snap\(fig\.T\)/.test(b)
         && /bsIt\('집값', snap\(fig\.P\)/.test(b)
         && /bsIt\('부대비용', Tg - snap\(fig\.P\)/.test(b)
         && /bsIt\('준비할 현금', snap\(fig\.C\)/.test(b)
         && /bsIt\('주담대', Tg - snap\(fig\.C\)/.test(b);
  })());
  ...
  /* 🔴 v25.54 신설 — **요약 문구도 좌우 두 쪽이다.**
     ⚠ 🔴 이 잠금은 **사보타주가 찾아냈습니다.** v25.54가 문구에 왼쪽 열을 넣고도 잠금을
       안 걸어서, 그 여섯 줄을 통째로 지워도 1019개가 전부 초록이었습니다 —
       **고친 것과 잠근 것이 어긋나 있었습니다**(원칙 127이 존재하는 이유).
     ⏹ 지키는 사실 : 문구가 총액을 말한 뒤 **그 돈이 어디로 가는지도 말한다.**
       v25.51~53의 문구는 「준비할 현금 · 주담대」 두 줄뿐이라 받는 사람이
       **얼마가 집값인지 못 봤습니다**(실기 캡쳐 2026.08.25).
     ⚠ 머리 두 개가 **화면·카드와 같은 말**인지까지 봅니다(원칙 91) — 세 매체가
       같은 이름을 써야 같은 표로 읽힙니다.
     ⚠ 네 값이 `heroFigures` 한 곳에서 오는지 봅니다 — 엔진 원값을 섞어 쓰면
       반올림이 갈려 문구가 화면과 1만원 다른 값을 말합니다(원칙 58). */
  tt('요약 문구가 좌우 두 쪽이다 (v25.54)', (()=>{
     const b = sumBody();
     /* 🔴 v26.0 재타겟(원칙 128) — 위 공유 카드와 같은 이유입니다. */
     return /'그럼 이 돈은 어디로 갈까요'/.test(b) && /'그 돈은 어디서 나올까요'/.test(b)
         && /집값 \$\{formatWon\(fig\.P/.test(b)
         && /부대비용 \$\{formatWon\(fig\.E/.test(b)
         && /준비할 현금 \$\{formatWon\(fig\.C/.test(b)
         && /주담대 \$\{formatWon\(fig\.L/.test(b);
  })());
  /* 🔴 v25.54 — **대출이 0이면 그 줄을 안 세웁니다**(원칙 124). 카드·화면과 같습니다(원칙 58). */
  tt('요약 문구가 대출 0에 주담대 줄을 안 세운다 (v25.54)',
     /if\(fig\.L > 0\) L\.push\(`· 주담대/.test(sumBody()));
  /* v24.15에서 쓰는 곳이 없어진 계산이 남아 있었습니다(원칙 47). */
  tt('공유 카드에 죽은 월 상환 계산이 없다', !/monthlyPaymentCalc/.test(repBody()));
  /* 🔴 v25.31 — **잠근 것이 이름이 아니라 순서였습니다**(원칙 48 · 128).
     이 검사의 뜻은 「공유 카드와 화면이 **같은 이름**을 쓴다」인데, 정규식이
     `준비할 현금 <b>${ownPct}%`로 **금액과 비율의 자리까지** 붙잡고 있었습니다.
  ...
  /* 🔴 v26.0 재타겟(원칙 128) — 오른쪽 열의 머리가 「그 돈은 어디서 나올까요」가 됐습니다.
     잠글 사실은 그대로 — **화면과 공유 카드가 같은 이름(준비할 현금 · 주담대)을 쓰는가.** */
  const LEGEND_LINE = (fs.readFileSync(FILE,'utf8')
     .match(/<p class="bs-cap">그 돈은 어디서 나올까요<\/p>[\s\S]*?<\/div>\s*<\/div>/) || [''])[0];
  ...
  /* 🔴 v25.37 — 잠근 사실은 **「화면 타일이 살아 있다」**입니다(요약 문구에서만 뺐다는 것).
     이름 앞머리(v25.37의 「주담대」)는 이 검사가 잴 사실이 아닙니다 — 대상을 옮겼습니다. */
  /* 🔴 v25.56 재타겟 — 위와 같은 이유로 가운데를 엽니다. 잠근 사실은 **「화면 타일이 살아 있다」**이고,
     낱말 순서는 그 사실에 안 들어 있습니다(원칙 48 · 128). */
  tt('화면 월 상환 타일은 그대로 있다 (v25.56 재타겟)',
     /class="tile-k">[^<]*매달 갚을[^<]*원리금</.test(M));
...
(() => {
  ...
  /* 🔴 v25.55 재타겟(원칙 128) — **흡수하는 칸이 `C`에서 `L`로 바뀌었습니다**(오너 결정).
     ⏹ 잠글 사실 자체는 안 바뀝니다 — **총액을 먼저 정하고 나머지를 빼서 만든다**(그래야 표가 맞습니다).
       바뀐 것은 **어느 칸이 반올림 오차를 떠안는가**입니다.
         v25.51~54 : T=ceil · P=floor · L=floor · E=T−P · **C=T−L**  → 현금이 최대 1만원 많게
         v25.55~   : T=ceil · P=floor · **C=ceil** · E=T−P · **L=T−C** → 대출이 최대 1만원 많게
     🔴 **`C = Math.ceil`이 이 판의 핵심입니다.** 이걸 `round`나 `floor`로 바꾸면
       ①입력한 현금과 다시 어긋나고 ②아래 「대출 초과 1만원 미만」 상한이 깨집니다. */
  tt('좌우 두 쪽이 반올림까지 맞는다 (v25.51 · v25.55 재타겟)', (()=>{
     const f = (UI.match(/function heroFigures\([\s\S]*?\n\}/)||[''])[0];
     return /Math\.ceil\(Math\.max\(0, c\.totalNeeded/.test(f)      /* 총액은 올림 */
         && /const P = Math\.floor/.test(f)
         && /const C = Math\.ceil\(Math\.max\(0, c\.cashNeeded/.test(f)
         && /const E = Math\.max\(0, T - P\)/.test(f)
         && /const L = Math\.max\(0, T - C\)/.test(f);
  })());
  /* 🔴 v25.55 신설 — **오너가 허용한 것은 「1만원」이지 「반올림 아무거나」가 아닙니다.**
     ⏹ 이 판은 이 앱에서 **유일하게** 「유리한 방향 오차」를 허용한 자리입니다
       (지침 : 유리하게 만드는 오차를 불리하게 만드는 오차보다 심각하게 다룬다).
       허용 근거는 오너 판단 하나 — 「만원은 대출 한도에서 극히 미미해」(2026.08.25).
     ⏹ 상한의 수학 : L − 실제대출 = (ceil(총액) − 총액) − (ceil(필요현금) − 필요현금)
       두 항 모두 [0,1)이므로 차는 **(−1, +1) 만원**입니다. 곧 **1만원 미만**이 보장됩니다.
       ⚠ 이 보장은 **C가 `ceil`이고 L이 `T − C`일 때만** 섭니다. 둘 중 하나라도
         따로 반올림하면 오차가 두 번 쌓여 1만원을 넘길 수 있습니다.
     ⏹ 실측(v25.55 · 504케이스 = 현금 14 × 소득 6 × 지역 3 × 보유 2) :
         P+E≠T 0건 · C+L≠T 0건 · C가 입력값과 다름 0건 · 음수 0건
         **대출 최대 초과 0.9172만원(9,172원)** — 현금 6억 1,250만 · 소득 8,000만 · 종로 · 무주택
     🔴 **`L`을 따로 반올림하는 코드가 들어오면 이 검사가 막습니다.** 그게 상한을 깨는 유일한 길입니다. */
  tt('대출 초과가 1만원 미만으로 묶여 있다 (v25.55)', (()=>{
     const f = (UI.match(/function heroFigures\([\s\S]*?\n\}/)||[''])[0];
     return /const L = Math\.max\(0, T - C\)/.test(f)
         && !/const L = Math\.(floor|ceil|round)\(/.test(f)
         && !/mortgageLoan/.test(f);        /* L이 엔진 대출값을 다시 읽으면 안 됩니다 */
  })());
  ...
  /* 🔴 v25.41 — **대상을 옮겼습니다**(원칙 128 · 148 — 같은 지시가 두 번 왔습니다).
     ⏹ 전에는 **문장 두 개**(라벨 + 꼬리)를 글자 그대로 잠갔습니다. 오너 지시는
       「구어체를 지우고 명사형으로」였고, 그 잠금이 오더를 막고 있었습니다.
  ...
  tt('집값 외에 드는 돈을 왼쪽 열이 말한다 (v25.51 재타겟)',
     /* 🔴 v26.0 재타겟(원칙 128) — 왼쪽 열 머리가 질문형이 됐습니다.
        ⚠ 「부대비용」은 **오너 결정으로 그대로**입니다 — 이 줄의 뒷조건은 안 건드렸습니다. */
     /<p class="bs-cap">그럼 이 돈은 어디로 갈까요<\/p>/.test(SRC) && />부대비용<span class="caret"/.test(SRC)
  ...
  /* 🔴 v25.53 재타겟 — **목적어를 넣었습니다**(오너 지시 — 「진짜 예산에 집을 넣어야겠어,
     목적어가 없어」). 「환상 없는 진짜 예산」 → **「환상 없는 내 집 예산」**.
     ⚠ 잠글 사실은 그대로입니다(원칙 128) — **그 자리에 각인이 있고, 옛 문장으로 안 돌아갔는가.**
       문자열을 바꿨으니 대상만 옮깁니다. 옛 문구가 되살아나면 빨간불입니다. */
  tt('첫 화면 각인이 교체된 카피다 (v25.53 재타겟)',
     /class="brandline" id="brand">환상 없는 내 집 예산<\/h1>/.test(SRC)
     && !/>환상 없는 진짜 예산</.test(SRC));
  /* ⏹ v25.53 — 크기 잠금을 **여기 새로 만들지 않았습니다.** 아래 「첫 화면 각인이 브랜드
     규격이다」(v25.40)가 이미 크기·굵기·색 셋을 함께 봅니다. 같은 사실에 잠금을 둘 두면
     다음 사람이 하나만 고치고 나머지를 잊습니다(원칙 55 — 없앤 중복이 새 중복을 만들지 않게).
     ⚠ 대신 그 잠금의 **대상을 `--t8` → `--t7`로 옮겼습니다**(원칙 128). */
...
(() => {
  ...
  /* 🔴 v25.41 — 면책 뒤에 **동선 버튼 자리(`#restartWrap`)가 왔습니다**(오너 지시 · 아래 v25.41 절).
     ⚠ 잠글 사실은 「면책이 마지막 요소다」가 아니라 **「맨 아래 각주가 면책 **하나**다」**입니다 —
       캡션 문단이 되살아나면 채권 이야기가 두 곳이 됩니다(원칙 58 · 84). */
  tt('맨 아래 각주가 면책 하나다', (()=>{
     const m = SRC.match(/<p class="foot" id="footNote"><\/p>[\s\S]{0,900}?<\/section>/);
     if(!m) return false;
     const tail = m[0].replace(/<!--[\s\S]*?-->/g,'');
     return !/id="caveatNote"/.test(SRC) && !/\.caveat\{/.test(SRC)
         && !/<p[^>]*>/.test(tail.replace(/<p class="foot" id="footNote"><\/p>/,''));
  })());
  ...
  tt('못 넣은 이유를 두 자리에서 같이 말한다', (()=>{
     const m = SRC.match(/notes\.innerHTML = hasLimit([\s\S]*?): '';/);
     const c = SRC.match(/\$\('costLead'\)\.innerHTML =([\s\S]*?);/);
     /* 🔴 v26.0 재타겟(원칙 128) — 어순 고침으로 「**조건마다 다른** 은행 우대금리」가
        「은행 우대금리는 **조건마다 달라요**」가 됐습니다(수식을 명사 뒤로).
        잠글 사실은 그대로 — **「안 넣었다」고만 하지 않고 왜인지 말하는가.** */
     return !!m && !!c && /조건마다 달라/.test(m[1]) && /할인율에 따라 달라져서/.test(c[1]);
  })());
  ...
  /* 🔴 v25.41 — **규칙을 한 자리에서 일반화했습니다.** v25.1은 `.chg b`에만 인라인을 줬고,
     그래서 **다른 항목의 설명 안 `<b>`는 여전히 블록**이라 실기에서 문장이 토막났습니다
     (오너 캡쳐 — 「주택담보대출 / 을 줄여 / 주담대 / 라고 적어요」).
     ⚠ 잠글 사실 : **설명(`span`) 안의 강조는 전부 인라인 · 제목(`> b`)만 블록.** */
  tt('설명 안 강조가 줄을 끊지 않는다 (v25.41)',
     /\.sheet-row span b\{display:inline/.test(RAW) && /\.sheet-row > b\{display:block\}/.test(RAW));
...
(() => {
  ...
  /* ⑤ 🔴 v25.54 재타겟 — **범례(`.lg`)라는 부품이 없어졌습니다.** 카드 몸통이 대차대조표가
     되면서 `.bar` · `.lg` 규칙을 통째로 지웠고(원칙 47), 이 세 검사는 **없는 것을 지키고**
     있었습니다. 잠금을 지우지 않고 **지키던 사실을 새 부품으로 옮깁니다**(원칙 128).
     ⚠ 없어진 부품의 잠금을 그대로 두면 「초록인데 아무것도 안 지키는」 검사가 됩니다 —
       그건 통과 개수만 늘리고 사고는 못 막습니다(원칙 149의 사촌). */
  /* ㉠ 옛 사실 「범례가 한 줄이다」 → **새 사실 「카드가 화면의 `.bs` 문법을 그대로 쓴다」.**
     v25.54가 새 클래스를 하나도 안 만들었다는 것이 이 판의 핵심입니다(지침 6-22 · 원칙 58). */
  tt('공유 카드가 화면의 대차대조표 문법을 그대로 쓴다 (v25.54 재타겟)', (()=>{
     const b = repBody();
     return /class="bs"/.test(b) && /class="bs-side"/.test(b) && /class="bs-cap"/.test(b)
         && /class="bs-it"/.test(b) && /class="bs-nm"/.test(b) && /class="bs-am"/.test(b)
         && !/report-mix-[a-z]|class="rmix/.test(b);
  })());
  /* ㉡ 옛 사실 「잘리는 대신 흐른다」 → **새 사실 「카드 금액을 잘라내지 않는다」.**
     440px 고정 캔버스라 잘리면 사용자가 **무엇이 잘렸는지 영영 못 봅니다.**
     ⚠ 실측(v25.54 · 열 177px · Range) : 가장 긴 후보 「20억 4,900만원」이 136.5px —
       지금은 여유가 40px 넘습니다. 그래도 **잘라내기 자체를 금지**합니다. 폭은 늙지만
       「잘리면 안 된다」는 안 늙습니다(원칙 166). */
  tt('공유 카드 금액을 잘라내지 않는다 (v25.54 재타겟)',
     !/\.bs-am\{[^}]*(white-space:nowrap|overflow:hidden|text-overflow)/.test(CSS)
     && !/\.bs-nm\{[^}]*(overflow:hidden|text-overflow)/.test(CSS));
  /* ㉢ 옛 사실 「범례에 괄호 금액이 없다」 → **그대로 삽니다. 대상만 `bsIt`로 옮깁니다.**
     🔴 이 검사가 지키는 것은 배치가 아니라 **재산 역산 방지**입니다(v24.15 실물 사고 —
       가진 돈 10억이 카드에 「(10억원)」으로 그대로 실려 재전달됐습니다).
       「준비할 현금」은 인테리어를 안 켜면 **가진 돈과 같습니다.**
     ⚠ 그래서 v25.54는 폭 근거가 만료됐는데도 **축약을 안 풀었습니다** — 남은 근거가
       개인정보이고, 그건 오너가 정할 문제입니다. */
  tt('공유 카드가 정확한 금액을 안 찍는다 (v24.21 · v25.60 재타겟)', (()=>{
     const f = (UI.match(/const bsIt = [\s\S]*?;\n/)||[''])[0];
     return f.length > 40 && !/formatWon|richWon|approxWon/.test(f) && /gridFmt/.test(f);
  })());
})();
...
(() => {
  ...
  /* 🔴 v26.0 재타겟 — **잠글 사실을 「명사구인가」에서 「권유·지시가 아닌가」로 옮겼습니다**
     (오너 결정 · 원칙 128 — 검사를 지우지 말고 대상을 옮긴다).
     ⏹ v25.3이 이 규칙을 만든 취지는 「궁금하시다면」 같은 **권유**를 막는 것이었습니다.
       사실을 묻는 질문형(「이 예산이면 어떤 집일까요」)은 그 취지에 안 걸립니다.
     🔴 **그리고 옛 겨눔은 우연히 맞고 있었습니다**(원칙 182) — 금지 목록에 「할까요」는 있는데
       「일까요」가 없어서, 새 제목이 **검사를 통과했지만 그건 규칙이 지켜져서가 아니었습니다.**
       그대로 두면 다음 판이 「궁금하시다면」을 넣어도 안 걸립니다. 그래서 다시 씁니다.
     ⚠ 개수는 그대로 셉니다 — 안 세면 제목이 통째로 사라져도 통과합니다(v25.21 근거). */
  tt('결과 카드 제목이 권유·지시가 아니다', (()=>{
     ...
     /* 🔴 v26.2 — 둘 → **셋**. 원리금 타일(`#payTile`)에 제목이 생겼습니다
        (「그럼 매달 얼마를 갚나요」 · 오너 결정). **검사를 지우지 않고 수만 옮깁니다**(원칙 128) —
        잠글 사실 둘(「제목이 통째로 사라지지 않았는가」 · 「지시·권유·조건절이 아닌가」)은 그대로입니다.
        ⚠ 새 제목의 어미 「갚나요」는 사실을 묻는 꼴이라 아래 세 정규식 어디에도 안 걸립니다 —
          **통과가 우연이 아닌지** 확인했습니다(원칙 182). */
     const t = [...SRC.matchAll(/<h2 class="card-title[^"]*">([^<]*)<\/h2>/g)].map(m=>m[1].trim());
     if(t.length !== 3) return false;
     /* ① 지시·권유 어미 — 사람에게 무엇을 하라고 시키는 꼴. */
     const 지시 = /(하세요|해보세요|보세요|주세요|보시죠|하시죠|합시다|하십시오)$/;
     /* ② 「~해볼까요」류 — 질문의 탈을 쓴 **권유**입니다. 「어떤 집일까요」(사실을 물음)와
        「같이 볼까요」(같이 하자)의 경계가 여기입니다. 앞에 동사가 붙은 「~ㄹ까요」만 잡습니다. */
     const 권유 = /(해볼까요|볼까요|갈까요|드릴까요|할까요)$/;
     /* ③ 조건절로 끝나 매달린 제목. */
     const 조건 = /(다면|라면|하면)$/;
     return t.every(x => !지시.test(x) && !권유.test(x) && !조건.test(x));
...
(() => {
  ...
  /* 🔴 v25.53 — **값이 아니라 의도를 잠급니다**(원칙 48). 이 검사의 주석이 스스로 말하는
     사실은 「스케일 밖 리터럴 0」인데, 정작 `--t8`이라는 **한 값**을 박고 있었습니다.
     그래서 크기를 한 칸 올리는 것만으로 빨간불이 났습니다 — 잠금이 개선을 막은 자리입니다.
     → **`--t*` 토큰이기만 하면 통과**로 바꿉니다. 어떤 칸인지는 위 「브랜드 규격」이 봅니다. */
  tt('첫 화면 활자가 스케일 그대로다',
     /\.brandline\{[^}]*font-size:var\(--t\d\)/.test(CSS)
  ...
  /* 🔴 v25.51 재타겟 — 화면 막대가 `.stack`에서 **열 막대**(`.bs-bar`)로 바뀌었습니다.
     ...
     🔴 v25.54 재타겟 — 카드가 **같은 문법**을 쓰던 것에서 **같은 클래스**를 쓰는 것으로
       바뀌었습니다(`.report-mix .bar` 규칙은 지웠습니다). 규칙이 하나가 됐으므로
       「둘이 같은가」를 잴 필요가 없어졌습니다 — 대신 **카드가 그 하나를 쓰는가**를 봅니다.
       ⚠ 이게 더 센 잠금입니다. 전에는 규칙 둘이 우연히 같은 값이면 통과였고,
         한쪽만 고쳐도 다른 쪽이 그대로 남아 조용히 갈라졌습니다(원칙 91이 막으려던 사고). */
  tt('막대가 화면 · 공유 카드에서 같은 규칙 하나다 (v25.54 재타겟)',
     /\.bs-bar\{[^}]*gap:2px/.test(CSS) && /\.bs-bar i\{[^}]*border-radius/.test(CSS)
     && /class="bs-bar"/.test(repBody())
     && !/\.report-mix \.bar\{/.test(CSS));
...
(() => {
  ...
  /* 🔴 v26.0 재타겟(원칙 128) — 제목이 **질문형**이 됐습니다(「이 예산이면 어떤 집일까요」 ·
     오너 결정 · 안 C). ⏹ 전에는 **문장 전체를 통째로** 잡고 있어 문구를 못 고쳤습니다(원칙 48).
     → 잠글 사실 둘만 남깁니다 : ① **예산이 이 목록을 거른다는 것을 제목이 말하는가**
       ② 위 미채택 둘(「노려볼」·「주요」)이 안 돌아왔는가 — ②는 바로 아래 짝 검사가 봅니다. */
  tt('실거래 카드 제목이 예산으로 거른 목록임을 말한다', (()=>{
     const m = SRC.match(/<h2 class="card-title">([^<]*)<\/h2>/g) || [];
     return m.some(x => /예산/.test(x));
  })(), (SRC.match(/<h2 class="card-title">([^<]*)<\/h2>/g)||[]).join(' / '));
  ...
  tt('Orphan 때문에 줄인 문장이 다시 길어지지 않았다', (()=>{
     ...
     /* 🔴 v26.0 재타겟(원칙 128) — 이 문장이 **어순 규칙으로 세 문장으로 갈렸습니다**
        (「세입자 보호를 위해 …」는 명사 앞 수식 열두 자였고, 5-2가 잰 390·430px 고아 줄).
        ⏹ 겨눔이 「세입자 보호를 위해」였는데 그 말 자체가 사라졌습니다.
        → 새 겨눔은 **방공제 설명 전체**이고, 상한은 **37자**입니다(고친 뒤 실측 · 전 53자).
          ⚠ 상한을 느슨하게 잡지 않았습니다 — 이 자리는 한 자만 늘어도 줄이 하나 더 생깁니다.
          🔴 인수인계 5-3의 안은 48자였는데 **360px에서 새 고아를 만들었습니다.** 자수가 아니라
            줄 상자를 재서 고른 값입니다(원칙 135 · 161 — 자수는 지표가 아닙니다). */
     return cap(/세입자를 보호하려고[^`]*?줄어요\./, 37)
...
(() => {
  ...
  /* 빈 값 문장이 **전체 대출**을 말하면 조건칩(「갚는 대출 월 N만원」)과 어긋납니다.
     ⚠ 비어 있는 것은 통과입니다 — 이름과 값이 이미 그 사실을 말하는 자리이기 때문입니다. */
  tt('원리금 타일의 빈 값이 전체 대출을 말하지 않는다', (()=>{
     const m = BARE.match(/tileMonthlySub'\)\.textContent = m>0 \? `[^`]+` : '([^']*)'/);
     return !!m && !/갚을 대출|모든 대출|대출이 없어요$/.test(m[1]) && (m[1] === '' || /주담대/.test(m[1]));
  })(), (()=>{ const m = BARE.match(/tileMonthlySub'\)\.textContent = m>0 \? `[^`]+` : '([^']*)'/);
     return m ? `'${m[1]}'` : '없음'; })());
  ...
  tt('원리금 타일이 범위를 이름과 부속 줄에서 두 번 말하지 않는다 (v25.37)', (()=>{
     ...
     return /주담대/.test(nm[1]) !== /주담대/.test(sb[1]);
  })(), (()=>{ const nm = BARE.match(/class="tile-k">([^<]*원리금)</);
     const sb = BARE.match(/tileMonthlySub'\)\.textContent = m>0 \? `([^`]+)`/);
     return `이름 '${nm?nm[1]:'?'}' / 부속 '${sb?sb[1]:'?'}'`; })());
...
(() => {
  ...
  /* 🔴 v25.61 재타겟 — **뒤집었습니다. 다만 지키던 사실은 그대로입니다.**
     ⏹ 옛 잠금 : 「결과에서 온 길을 초안에 저장하지 않는다」.
       근거 — 「탭을 닫았다 돌아온 사람의 버튼까지 『결과 보기』가 되는데,
       **그 사람에게는 돌아갈 결과가 없다**」. **이 근거는 옳고, 안 늙었습니다.**
     🔴 그런데 그 근거를 **저장을 막아서** 지키고 있었고, 대가가 컸습니다 —
       조건칩으로 퍼널에 들어간 사람이 **새로고침 한 번에 갇힙니다**(오너 실기 지적).
       모바일 브라우저는 탭을 뒤로 보내면 페이지를 다시 읽으므로 **사용자가 아무것도
       안 해도** 납니다.
     → **저장은 하되 `ready()`로 버튼을 막습니다.** 근거를 더 좁게 지키는 길입니다.
       `backToResult`는 이미 `if(!ready()) return;`이었으므로, 안 막으면
       **보이는데 눌러도 안 되는 버튼**이 됩니다(원칙 91 — 보이는 것과 되는 것을 맞춥니다).
     ⚠ 원칙 172 — 근거가 안 늙었을 때 뒤집는 것은 **결론이 아니라 방법**입니다. */
  tt('돌아갈 결과가 없으면 복귀 버튼을 안 낸다 (v25.15 · v25.61 재타겟)',
     /tr\.hidden = !!onResult \|\| !S\.fromResult \|\| !ready\(\);/.test(UI));
...
(() => {
  ...
  /* ── v25.41 ① 값이 없으면 **카드도 없다** (오너 실기 지적) ──
     ⏹ 대출 0원인데 「주담대 매달 갚을 원리금 없어요 / 소득 대비 부담 —」가 빈 카드로 서 있었습니다.
     ⚠ 잠글 것은 「숨긴다」가 아니라 **「값이 있을 때만 선다」**입니다 — 조건식을 봅니다(원칙 149). */
  tt('대출이 없으면 원리금 카드가 서지 않는다 (v25.41)', (()=>{
     const bare = CSSB;  /* 주석 걷은 소스 — 아래는 마크업·스크립트를 같이 봅니다 */
     const raw = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'').replace(/<!--[\s\S]*?-->/g,'');
     return /<div class="tile" id="payTile">/.test(raw)
         && /\$\('payTile'\)[\s\S]{0,60}hidden = !\(m > 0\)/.test(raw);
  })());
  /* 🔴 v26.2 신설 — **원리금 타일이 제목을 갖는다.**
     ⚠ 위 「결과 카드 제목이 권유·지시가 아니다」는 제목을 **셋 셉니다.** 개수만 세면
       다른 카드에 제목이 하나 더 생기고 이 타일 제목이 사라져도 셋이라 통과합니다 —
       **어느 카드의 제목인지**를 여기서 잠급니다(원칙 142 · 149).
     ⚠ 문장을 통째로 잠그지 않습니다(원칙 48). 잠글 사실은
       ① `#payTile` 안에 `.card-title`이 있다 ② 그 제목이 **매달 갚는 것**을 묻는다
       ③ 왼쪽 칸 이름(「매달 갚을 주담대 원리금」)이 **그대로 살아 있다** — 제목이 생겼다고
         칸 이름을 지우면 「무슨 대출인가」가 화면에서 사라집니다(v25.13 · v25.37이 정한 사실). */
  tt('원리금 타일이 제목을 갖고 칸 이름은 그대로다 (v26.2)', (()=>{
     const raw = fs.readFileSync(FILE,'utf8').replace(/\/\*[\s\S]*?\*\//g,'').replace(/<!--[\s\S]*?-->/g,'');
     const tile = (raw.match(/<div class="tile" id="payTile">[\s\S]*?<div class="payrow">/)||[''])[0];
     const t = (tile.match(/<h2 class="card-title">([^<]*)<\/h2>/)||['',''])[1];
     return !!t && /매달/.test(t) && /갚/.test(t)
         && /<p class="tile-k">매달 갚을 주담대 원리금<\/p>/.test(raw);
...
(() => {
  ...
  /* 🔴 v25.60 재타겟 — 축약 함수가 `eokShort`(전역)에서 **`gridFmt`(카드 안 지역)**으로
     바뀌었습니다. 잠글 사실은 그대로입니다 — **축약이 화면으로 안 퍼진다.**
     ⚠ 이제 더 강합니다 : `gridFmt`는 카드를 그리는 함수 **안에서 선언**되므로
       화면 코드에서는 **부를 수조차 없습니다.** 전역이던 시절에는 규율로만 막았습니다. */
  tt('화면이 축약 금액을 안 쓴다 (v25.48 · v25.60 재타겟)', (()=>{
     const at = (bare.match(/gridFmt/g) || []).length;   /* 선언 1 + 쓰는 곳 1 */
     return at === 2 && !/eokShort/.test(bare)
         && /const gridFmt = m => \{/.test(repBody());   /* 카드 안에서만 삽니다 */
  })());
  ...
  /* 🔴 v25.60 재타겟 — 축약 함수가 `eokShort`(전역)에서 **`gridFmt`(카드 안 지역)**으로
     바뀌었습니다(원칙 128 · 175). 아래 두 검사가 지키는 사실은 그대로입니다. */
  const eok = (bare.match(/const gridFmt = m => \{[\s\S]*?\n  \};/) || [''])[0];
  const pol = (bare.match(/function policyAmount[\s\S]*?\n\}/) || [''])[0];
  tt('축약 표기와 정책 표기가 서로 독립이다 (v25.30 · v25.60 재타겟 · 원칙 132)',
     !!eok && !!pol && !/policyAmount/.test(eok) && !/gridFmt/.test(pol), eok.slice(0, 120));
  /* ⚠ 그리고 **1억 미만은 억으로 안 줄입니다** — 「0.8억」은 이 나라에서 읽는 말이 아닙니다.
     그때는 앱의 금액 표기(formatWon)로 떨어집니다. */
  /* 🔴 v25.60 재타겟 — 단위가 **원**에서 **만원**으로 바뀌었습니다(1억 = 10000만원).
     지키는 사실은 그대로 — **1억 미만은 억으로 안 줄이고 만원으로 적는다.**
     ⚠ v25.60이 눈금을 도입하면서도 이 규칙은 안 건드렸습니다 : 부대비용 7,278만원은
       8,000만원으로 반올림되지만 **「0.8억」이 되지는 않습니다.**
       그래도 합은 맞습니다 — 177,000 + 8,000 = 185,000만원(= Tg). */
  tt('1억 미만은 앱의 금액 표기로 떨어진다 (v25.30 · v25.60 재타겟)',
     /< 10000/.test(eok) && /만원/.test(eok) && !/0\.\d억/.test(eok), eok.slice(0, 160));

  /* ④ 🔴 **이 축약을 범례 밖으로 퍼뜨리지 않습니다.** 최대 500만원까지 실제와 다르게 보이는
     표기라, 정확한 금액을 말해야 하는 자리(영수증 · 히어로 · 공유 카드)에 들어가면 안 됩니다. */
  const all = (bare.match(/gridFmt\(/g) || []).length;   /* v25.60 재타겟 */
  ...
  /* ⚠ v25.60 — 선언이 `const gridFmt = m => {`라 **`gridFmt(` 형태가 아닙니다.**
     그래서 「선언 한 줄을 뺀다」가 필요 없어졌습니다. 안 빼면 −1이 되어 조용히 어긋납니다. */
  const spread = (bare.match(/gridFmt\(/g) || []).length;
  /* ⚠ **선언(`function eokShort(won){`)이 같은 글자입니다** — 처음에 그걸 같이 세서
     「공유 카드 2곳」이 나왔습니다. 템플릿 자리(`${…}`)만 셉니다(원칙 152). */
  const inCard = (bare.match(/\$\{gridFmt\(man\)\}/g) || []).length;
  ...
  tt('🔴 축약 표기가 공유 카드에만 있다 (v25.48 · v25.60 재타겟)',
     spread === inCard && inCard === 1 && !/row\('집값', gridFmt/.test(bare),
...
(() => {
  ...
  /* ① 🔴 **금액이 비율보다 앞에** 옵니다(오너 지시 v25.31).
     ⚠ 잠글 것은 「금액이 먼저다」라는 **자리**입니다 — 오너가 두 번 지시한 방향이고,
       근거는 「이 줄의 주인공은 금액」입니다(원칙 148 — 잠금에 근거를 같이 적습니다).
     ⚠ 문자열 전체를 잠그지 않습니다. 세 갈래 각각에서 **금액이 %보다 왼쪽인가**만 봅니다. */
  const 앞선다 = (chunk) => {
  ...
  tt('평형 값 자리에 「모름」이 없다 (v25.32)',
     !/모름/.test(bare), (bare.match(/.{0,40}모름.{0,40}/) || [''])[0]);
  /* ④ 🔴 그 값을 만드는 **두 자리가 같은 식**인가 — 갈리면 끄는 순간 글자가 바뀝니다(원칙 58). */
  tt('평형 값을 만드는 두 자리가 같은 식이다 (v25.32)',
     pyeongWrites.length === 2 && pyeongWrites[0] === pyeongWrites[1],
     pyeongWrites.join('  ↔  ') || '🔴 못 찾음');
  /* ⑤ 🔴 **값이 없을 때 줄 높이를 지키는가.**
     잠글 것은 「\00a0을 쓴다」가 아니라 **「빈 값에도 자리가 있다」**입니다(원칙 149).
     ⏹ 근거(3폭 실측): 이 규칙이 없으면 그 줄이 30 → 19.5px로 주저앉고, 슬라이더를 처음
       잡는 순간 값이 뜨면서 **손가락 밑에서 10.5px 내려갑니다**(원칙 34). */
  const empt = (CSS.match(/\.slider-head \.v:empty::after\{[^}]*\}/) || [''])[0];
  tt('🔴 평형 값이 비어도 줄 높이를 지킨다 (v25.32 · 근거: 30 → 19.5px · 슬라이더 10.5px 이동)',
     /content:/.test(empt), empt || '🔴 규칙이 없습니다');
  /* ⑥ 마크업 기본값도 비어 있는가 — JS가 손대기 전 화면에 「모름」이 남아 있으면 안 됩니다. */
  tt('마크업 기본값도 비어 있다 (v25.32)',
     /<span class="v" id="pyeongVal"><\/span>/.test(bare),
...
/* ══════════════════════════════════════════════════════════════════════
   v25.73 — 표시 금리 기본값의 **배선과 근거**

   🔴 v25.72까지 `RATE_BAND` · `KFB_*` · `RATE_DEFAULT` 전체가 **검사 밖**이었습니다.
      `RATE_DEFAULT`는 선언만 되고 아무 데서도 안 불렸고, `D.rate`가 같은 값 `5.4`를
      **손으로** 들고 있었습니다 — 근거 주석 네 줄이 아무것도 안 지키는 자리에 붙어
      있었던 셈입니다. 사보타주73이 이 자리를 잡습니다.
   ══════════════════════════════════════════════════════════════════════ */
(() => {
  const bare = fs.readFileSync(FILE, 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, '').replace(/<!--[\s\S]*?-->/g, '');

  /* ① 배선 — 기본 금리는 근거가 적힌 상수에서 온다. 리터럴을 다시 박으면 빨간불. */
  tt('표시 금리 기본값이 RATE_DEFAULT에서 온다 (v25.73 · 원칙 58)',
     /const D = \{\s*rate:\s*RATE_DEFAULT\s*,/.test(bare),
     (bare.match(/const D = \{[^}]*\}/) || ['(못 찾음)'])[0].slice(0, 90));

  /* ② 그 상수가 실제로 선언되어 있고, 쓰이는 곳보다 앞이다. */
  tt('RATE_DEFAULT가 쓰이는 곳보다 먼저 선언된다', (() => {
     const dec = bare.indexOf('const RATE_DEFAULT =');
     const use = bare.indexOf('rate:RATE_DEFAULT');
     return dec > -1 && use > -1 && dec < use;
  })());

  /* ③ 🔴 값 잠금 — **근거 값을 함께 답니다**(원칙 148).
        5.4는 규칙에서 유도되지 않습니다. 두 공시 사이에서 오너가 고른 값입니다.
          아래 = KFB 5대 은행 평균 4.50 (은행연합회 · 2026년 6월 취급분)
          위   = RATE_BAND.variable.hi 6.88 (언론 집계 · 2026.07.20)
        🔴 낮출수록 집값이 **크게** 나옵니다 — 0.4%p당 약 1,600만원(v25.73 실측).
          원칙 28의 방향이라, 값을 바꾸려면 이 잠금과 근거 주석을 **같이** 고치십시오. */
  const num = re => { const m = bare.match(re); return m ? parseFloat(m[1]) : NaN; };
  const def  = num(/const RATE_DEFAULT = ([\d.]+);/);
  const top5 = num(/const KFB_TOP5_AVG = ([\d.]+);/);
  const hi   = num(/variable:\s*\{lo:\s*[\d.]+,\s*hi:\s*([\d.]+)/);

  tt('기본 금리가 5.4다 (v25.73 · 근거 4.50 ↔ 6.88 사이)',
     def === 5.4, `RATE_DEFAULT = ${def}`);

  tt('기본 금리가 은행 평균보다 낮지 않다 (보수 방향 · 원칙 28)',
     Number.isFinite(def) && Number.isFinite(top5) && def >= top5,
     `기본 ${def} · KFB 5대 평균 ${top5}`);

  tt('기본 금리가 게시 구간 위쪽을 넘지 않는다',
     Number.isFinite(def) && Number.isFinite(hi) && def <= hi,
     `기본 ${def} · 변동형 상단 ${hi}`);

  /* ⏹ 🔴 v25.74 — **보금자리론 값 4개를 잠그지 않습니다.**

       v25.73이 u · 아낌e · t 금리 18개와 공시일 일치를 잠갔다가 **뺐습니다.**
       값이 틀려서가 아닙니다 — 2026.08.31에 주금공 원문과 전수 대조해 전부 맞았습니다.

       뺀 이유 : `BOGEUM_BASE`는 **화면에 안 닿습니다.** `ctx.loanChoice`가 `'bank'`
       상수라 `POLICY_DEFS.bogeumjari`에 도달 못 하고 `bogeumRate()`는 호출처가 0곳입니다.
       정책대출 화면은 **오너 결정으로 「하지 않습니다」**(2026.08.27).

       🔴 도달 불가 코드를 잠그면 **죽은 잠금**입니다(원칙 148) —
         사용자가 못 보는 값을 지키느라, 정작 「이 코드는 안 닿아서 안전하다」는
         근거가 흐려집니다. 잠금은 **화면에 닿는 값**에만 겁니다(신설 원칙 191).

       → 정책대출 화면을 만들기로 한 판에서, 마크업 · 함수 · 검사를 **한 번에**
         여십시오(원칙 141). 그때 이 자리에 잠금을 되살리면 됩니다.
         확인해 둔 값(2026.08.31 · 9월 동결) : u 5.00/5.10/5.15/5.20/5.25/5.30 ·
         아낌e 4.90/5.00/5.05/5.10/5.15/5.20 · t는 u와 같음 · 우대 1.0%p · 규제 가산 0.2%p */
})();

/* ═══ v25.75 — 주담대가 0원일 때 **원인별로 다른 말을 한다** ═══════════
   ⏹ v25.74까지 `bindingTip`의 첫 갈래가 `L.LTV===0 || c.mortgageLoan<=0`이라
     **모든 0을 삼켜** 「규제지역 + 유주택」 한 문장으로 답했습니다.
     `tools/zeroloan.cjs` 전수 : 22개 갈래 중 **11개가 틀린 이유**를 댔습니다.
   🔴 잠그는 것은 **문장이 아니라 사실**입니다(원칙 48 · 128) —
     「이 갈래가 저 원인에 붙어 있는가」와 「원인을 안 묻고 삼키지 않는가」.
   ═══════════════════════════════════════════════════════════════ */
(() => {
  const F = (UI.replace(/\/\*[\s\S]*?\*\//g, '')
              .match(/function bindingTip\([\s\S]*?\n\}/) || [''])[0];

  tt('한도 안내를 읽어 왔다 (v25.75)', !!F && F.length > 200);

  /* 🔴 이 판의 본체 — 원인을 안 묻고 모든 0을 삼키던 조건이 사라졌는가.
     ⚠ 「`mortgageLoan<=0`을 안 쓴다」가 아닙니다(그 갈래는 여전히 필요합니다).
       잠그는 사실은 **그것이 규제·유주택 문구와 같은 줄에 있지 않다**입니다. */
  tt('대출 0을 규제·유주택 문구가 삼키지 않는다 (v25.75)', (() => {
     const line = (F.match(/if\s*\([^)]*L\.LTV\s*===\s*0[^)]*\)/) || [''])[0];
     return !!line && !/mortgageLoan/.test(line);
  })());

  /* 규제 · 유주택 문구는 **LTV가 0인 갈래에만** 붙어 있어야 합니다. */
  tt('규제·유주택 문구가 LTV 0 갈래에 붙어 있다 (v25.75)', (() => {
     const i = F.indexOf('L.LTV === 0') >= 0 ? F.indexOf('L.LTV === 0') : F.indexOf('L.LTV===0');
     const j = F.indexOf('규제지역에서 집이 이미 있으면');
     return i > 0 && j > i && (j - i) < 200;
  })());

  /* 🔴 갚는 대출이 한도를 다 먹은 갈래 — **DSR 한도가 0인가**를 봅니다.
     `c.binding`이 아니라 한도 값을 보는 것이 정확합니다(원칙 84 — 선언이 아니라 실제 값). */
  tt('갚는 대출 갈래가 DSR 한도 0을 본다 (v25.75)',
     /L\.DSR\s*===\s*0/.test(F) && /갚고 있는 대출이 한도를 다 썼어요/.test(F));

  /* ⚠ 원인을 소득이라고 말하면 안 됩니다 — 기존 대출이 0이면 DSR 한도는 0이 될 수 없습니다.
     그 사실을 잠급니다(원칙 39 — 이름이 값보다 더 약속하지 않기). */
  tt('갚는 대출 갈래가 원인을 소득으로 돌리지 않는다 (v25.75)', (() => {
     const seg = (F.match(/갚고 있는 대출이 한도를 다 썼어요[\s\S]{0,220}/) || [''])[0];
     return !!seg && !/소득이 한도를 정했어요|소득으로는/.test(seg);
  })());

  /* 방공제 갈래 — `roomCut`이 실제로 있을 때만 그렇게 말합니다. */
  tt('방공제 갈래가 roomCut을 보고 말한다 (v25.75)',
     /c\.roomCut\s*>\s*0/.test(F) && /방공제를 빼면 남는 게 없어요/.test(F));

  /* ⚠ **금액은 여기서 안 적습니다** — 바로 아래 roomTip이 이미 말합니다(원칙 43). */
  tt('방공제 갈래가 금액을 두 번 적지 않는다 (v25.75)', (() => {
     const seg = (F.match(/방공제를 빼면 남는 게 없어요[\s\S]{0,160}/) || [''])[0];
     return !!seg && !/roomCut\s*\)|formatWon\(\s*c\.roomCut/.test(seg);
  })());

  /* 원인이 넷 다 아닌 0에서 **원인을 지어내지 않는가.** */
  tt('원인을 못 짚으면 원인을 지어내지 않는다 (v25.75)', (() => {
     /* ⚠ **그 갈래가 돌려주는 문자열만** 봅니다. 뒤 갈래까지 훑으면 다음 문장의
        「소득」에 걸려 아무것도 안 재고 빨간불이 납니다(원칙 152 — 검사의 전처리). */
     const seg = (F.match(/이 조건에서는 주담대가 나오지 않아요[^`]*/) || [''])[0];
     return !!seg && !/규제지역|갚고 있는|방공제|소득/.test(seg);
  })());

  /* 🔴 갈래 순서 — LTV 0이 먼저입니다. 겹치는 사람에게는 그 문구가 참입니다. */
  tt('LTV 0을 먼저 본다 (v25.75)', (() => {
     const a = F.search(/L\.LTV\s*===\s*0/);
     const b = F.search(/c\.mortgageLoan\s*<=\s*0/);
     return a > 0 && b > 0 && a < b;
  })());

  /* ⚠ 대출이 0이 아닌 갈래들은 **한 줄도 안 건드렸습니다.** 되살아나지 않는지 봅니다. */
  tt('DSR·구간한도·LTV 설명 갈래가 그대로다 (v25.75)',
     /소득이 한도를 정했어요/.test(F) && /정부 규제 기준이에요/.test(F)
     && /주담대 비율\(LTV\)이 한도를 정했어요/.test(F));
})();

/* ═══ v25.76 — 「그럼 그 남는 돈으로 위를 사면 되나」에 답한다 ═══════════
   ⏹ v25.64가 **미룬 안 B**입니다(「경계 위 집값을 다시 찾아야 하므로 계산 표시 판」).
     오너 가족이 실기에서 물었고 오너도 답을 못 했습니다 — 화면이 답을 안 갖고 있었습니다.
   🔴 잠그는 것은 **문장이 아니라 사실**입니다(원칙 48 · 128) :
     ①경계 위 가장 싼 지점에서 잰다 ②엔진에서 온다 ③손으로 적은 숫자가 없다
     ④0 이하면 아무 말도 안 한다 ⑤화면이 쓰는 것과 같은 전제를 쓴다.
   ═══════════════════════════════════════════════════════════════ */
(() => {
  const F = (UI.replace(/\/\*[\s\S]*?\*\//g, '')
              .match(/const cliff = \(\(\) => \{[\s\S]*?\n  \}\)\(\);/) || [''])[0];

  tt('절벽 안내를 읽어 왔다 (v25.76)', !!F && F.length > 200);

  /* 🔴 **글자가 소스에 있는가가 아니라, 그 값이 돌려주는 문자열에 실리는가**를 봅니다.
     ⏹ 처음에 `/넘으려면 현금이/.test(F)`만 봤다가 **사보타주 ㉠에 안 물었습니다** —
       `need`를 만들어 놓고 반환문에서 빼도 그 글자는 소스에 그대로 남습니다(원칙 122). */
  tt('「넘으려면 N 더」가 화면에 실린다 (v25.76)',
     /넘으려면 현금이/.test(F) && /더 있어야 해요/.test(F)
     && /return `[\s\S]*?\$\{need\}[\s\S]*?`;/.test(F));

  /* 🔴 경계 **위** 지점에서 잽니다 — 경계값 자체로 재면 지금 값이 그대로 나와 0이 됩니다. */
  tt('경계를 넘긴 지점에서 잰다 (v25.76)', /B\[i\]\.upTo\s*\+\s*\d+/.test(F));

  /* 🔴 값이 **엔진에서** 옵니다 — 화면이 따로 셈하면 두 번째 정의입니다(원칙 58). */
  tt('필요 현금을 엔진에서 가져온다 (v25.76)',
     /calcCosts\(/.test(F) && /cashNeeded/.test(F));

  /* ⚠ 화면이 지금 계산에 쓴 것과 **같은 전제**여야 합니다(원칙 91). */
  /* 🔴 **집값 말고는 아무것도 안 갈아끼웁니다.** 하나라도 덮으면 화면이 지금 쓴 전제와
     달라져 두 숫자가 서로를 반박합니다(원칙 91).
     ⏹ 처음에 `...ctx()`가 있는지만 봤다가 **사보타주 ㉢에 안 물었습니다** —
       뒤에 `otherDebtMonthly: 0`을 더해도 통과했습니다. 이제 **키 목록**을 봅니다. */
  tt('집값 말고는 전제를 안 갈아끼운다 (v25.76)', (() => {
     const call = (F.match(/calcCosts\(\{[^}]*\}\)/) || [''])[0];
     if(!/\.\.\.ctx\(\)/.test(call)) return false;
     const keys = (call.match(/(\w+)\s*:/g) || []).map(x => x.replace(/\s*:$/, ''));
     return keys.length === 1 && keys[0] === 'price';
  })());

  /* ⚠ 0 이하이거나 못 재면 **아무 말도 안 합니다** — 지어내지 않습니다(원칙 39 · 115). */
  /* 🔴 **못 재면 비웁니다.** `catch`가 있는지가 아니라 **그 안이 무엇을 하는가**를 봅니다.
     ⏹ 처음에 `catch`의 존재만 봤다가 **사보타주 ㉣에 안 물었습니다** — 잡아 놓고
       거기서 문장을 만들어도 통과했습니다. 짐작으로 채우는 것이 정확히 그 모양입니다. */
  tt('못 재거나 0 이하면 말하지 않는다 (v25.76)', (() => {
     if(!/if\(gap > \d+\)/.test(F)) return false;
     const body = (F.match(/catch\([^)]*\)\s*\{([^}]*)\}/) || ['',''])[1];
     return /=\s*''/.test(body) && !/넘으려면|더 있어야/.test(body);
  })());

  /* 🔴 **손으로 적은 금액이 없습니다** — 경계도 필요 현금도 값에서 옵니다(원칙 84). */
  tt('절벽 안내에 손으로 적은 금액이 없다 (v25.76)', (() => {
     const txt = (F.match(/`[^`]*넘으려면[^`]*`/g) || []).join(' ')
               + (F.match(/`[^`]*낮아져요[^`]*`/g) || []).join(' ');
     return !!txt && !/\d+억|\d+만원/.test(txt);
  })());

  /* ⚠ 이 줄은 **경계에 앉았을 때만** 뜹니다 — 그 조건이 사라지지 않았는가. */
  tt('경계에 앉았을 때만 뜬다 (v25.76)',
     /구간한도/.test(F) && /findIndex/.test(F) && /!B\[i \+ 1\]/.test(F));

  /* ⚠ 말투 — 사실만. 지시하지 않습니다. */
  tt('절벽 안내가 지시하지 않는다 (v25.76)', (() => {
     const txt = (F.match(/`[^`]*`/g) || []).join(' ');
     return !/사세요|하세요|권|추천|낮추세요|줄이세요/.test(txt);
  })());
})();

/* ═══ v25.77 — 서울 지역 안내는 **응답이 아니라 그룹의 성질**이다 ═══════════
   오너 지적 : 「서울은 같은 투기과열지구이니깐 같은 말이 나오는 게 맞긴 한데,
   구 선택하면 아래 같은 안내문구가 나오는 거면 굳이 거기에 위치할 필요가 있나」.
   🔴 잠그는 것은 **자리**가 아니라 **왜 그 자리인가**입니다(원칙 48 · 128) —
     ①서울 안내는 격자에 안 들어간다 ②경기·인천은 그대로 칩 뒤다(v25.14의 근거가 여전히 참)
     ③같은 말이 두 번 뜨지 않는다.
   ═══════════════════════════════════════════════════════════════ */
(() => {
  const R = (UI.replace(/\/\*[\s\S]*?\*\//g, '')
              .match(/function regionPane\(\)\{[\s\S]*?\n\}/) || [''])[0];

  tt('지역 패널을 읽어 왔다 (v25.77)', !!R && R.length > 300);

  /* 🔴 서울 갈래가 격자에 안내를 **안 넘깁니다**(넘기면 칩 뒤로 들어가 격자를 쪼갭니다). */
  tt('서울 격자에 안내를 넘기지 않는다 (v25.77)', (() => {
     const line = (R.match(/S\.group==='seoul'[\s\S]{0,400}?tagGrid\(picksOf\(SEOUL\)[^)]*\)/) || [''])[0];
     return !!line && /tagGrid\(picksOf\(SEOUL\), *false, *''\)/.test(line);
  })());

  /* 서울 안내가 **제목과 격자 사이**에 있습니다. */
  tt('서울 안내가 격자 앞에 선다 (v25.77)', (() => {
     const line = (R.match(/S\.group==='seoul'[\s\S]{0,500}?tagGrid/) || [''])[0];
     /* 🔴 v26.1 재타겟(원칙 128) — 제목이 지시문에서 질문형이 됐습니다.
        잠글 사실은 그대로 — **제목 → 안내 → 격자** 순서입니다. */
     const a = line.indexOf('어느 자치구인가요');
     const b = line.indexOf('서울은 전 구역이');
     const c = line.indexOf('tagGrid');
     return a >= 0 && b > a && c > b;
  })());

  /* ⚠ 문구 — 「그 구만 규제」로 읽히지 않게 **전 구역**임을 말합니다(원칙 39). */
  tt('서울 안내가 전 구역임을 말한다 (v25.77)',
     /서울은 전 구역이 규제지역이에요/.test(R));

  /* 🔴 같은 말이 두 번 뜨지 않게, 서울에서는 `tag` 갈래 자체를 비웁니다.
     안 비우면 이 절 끝의 「끝에 붙입니다」가 옛 문구를 한 번 더 답니다(원칙 43). */
  tt('서울에서는 tag 갈래를 비운다 (v25.77)',
     /const tag = \([^)]*S\.group !== 'seoul'[^)]*\)/.test(R));

  /* ✅ 경기·인천은 **그대로**입니다 — 거기서는 안내가 고른 것에 따라 갈립니다. */
  tt('경기·인천 안내는 칩 뒤 그대로다 (v25.77)', (() => {
     const n = (R.match(/tagGrid\(picksOf\(S\.sido\), *false, *tag\)/g) || []).length;
     return n >= 2;                      /* 경기·인천 갈래 · 그 외 지역 갈래 */
  })());

  /* ⚠ 「수도권이에요」·「규제지역이에요」 두 문장은 **한 벌만** 남아 있습니다. */
  tt('지역 안내 문장이 한 벌씩이다 (v25.77)', (() => {
     const bare = UI.replace(/\/\*[\s\S]*?\*\//g, '');
     return (bare.match(/<b>규제지역이에요\.<\/b>/g) || []).length === 1
         && (bare.match(/<b>수도권이에요\.<\/b>/g) || []).length === 1
         && (bare.match(/<b>서울은 전 구역이 규제지역이에요\.<\/b>/g) || []).length === 1;
  })());
})();
