...
(() => {
  ...
  tt('첫 화면 각인이 브랜드 규격이다 (v25.40 · v25.53 재타겟)',
     /\.brandline\{[^}]*font-size:var\(--t7\)/.test(css2)
...
(() => {
  ...
  tt('🔴 공유 카드가 축약 표기를 쓴다 (v25.36 · v25.54 재타겟)',
     /const bsIt = \(name, won, pct\) =>/.test(repBody())
     && /eokShort\(won\)/.test(repBody()),
...
(() => {
  ...
  tt('공유 카드가 정확한 금액을 안 찍는다 (v24.21 · v25.54 재타겟)', (()=>{
     const f = (UI.match(/const bsIt = [\s\S]*?;\n/)||[''])[0];
     return f.length > 40 && !/formatWon|richWon|approxWon/.test(f) && /eokShort/.test(f);
  })());
})();
